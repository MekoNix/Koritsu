...
def test_первые_модули_на_месте(клиент, хозяин):
    """Отчёты, задания, блок-схемы и UML — то, что уже работает. Порядок —
    порядок сайдбара, и он не случайный: отчёты первые, потому что ради них
    всё, а задания — второй способ получить ту же работу."""
    assert [m["id"] for m in клиент.get("/api/modules").json()] == [
        "reports", "kadai", "flowcharts", "uml"]
...
def test_описания_модулей_английские_и_свои(app):
    ...
    свои = {"modules", "flowcharts", "uml", "artifacts", "kadai"}
    for путь, метод, операция in операции(app.openapi()):
