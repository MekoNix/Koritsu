...
# Исключения из форм выше: адреса, которые выбрали не мы. Мост распознавания
# рукописи (`board`) стоит там, куда стучится чужая библиотека, — путь зашит в
# её сборку и не настраивается, а ставить рядом второй, «правильный», значило бы
# завести адрес, по которому никто не придёт. Список поимённый и короткий: он
# существует затем, чтобы каждое такое исключение было видно глазами.
ЧУЖИЕ_АДРЕСА = {"board": ("/v4.0/iink/",)}
...
def test_первые_модули_на_месте(клиент, хозяин):
    """Отчёты, задания, доска, блок-схемы и UML — то, что уже работает. Порядок
    — порядок сайдбара, и он не случайный: отчёты первые, потому что ради них
    всё, задания — второй способ получить ту же работу, а доска — третий, где
    задачу решают от руки."""
    assert [m["id"] for m in клиент.get("/api/modules").json()] == [
        "reports", "kadai", "board", "flowcharts", "uml"]


def test_пустых_заготовок_не_держим():
    """Решение плана: `cards` и `asm` появятся строкой, когда появится
...
def test_описания_модулей_английские_и_свои(app):
    ...
    свои = {"modules", "flowcharts", "uml", "artifacts", "kadai", "board"}
    for путь, метод, операция in операции(app.openapi()):
