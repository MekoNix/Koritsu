...
def test_превью_записывается_у_названного_запуска(app, клиент, работа):
    """Идентификатор картинки ложится в ту запись, которую назвали.

    Запись называется аргументом, а не берётся из задания: документ собирают
    два разных прогона — шаблонная сборка пишет картинку отчёту, а стадия
    «сборка» сценария — решению. Взяли бы мы поле сами, работа с двумя
    отчётами показывала бы одну страницу на обеих карточках.
    """
    import orchestrator
    ...
    art = запомнить_превью(_Контекст(app, pid), проект, pdf,
                           запуск=отчёт["id"])
    assert art
    with app.state.db.session_scope() as s:
...
def test_превью_не_пишется_без_запуска(app, клиент, работа):
    """Сборка не из записи журнала картинку не запоминает: записать её некуда."""
    import orchestrator
