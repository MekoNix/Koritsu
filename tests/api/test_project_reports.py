...
def test_отчёт_без_бланка(клиент, работа):
    """Отчёт заводится и без бланка: документ строится с нуля."""
    отчёт = завести_отчёт(клиент, работа["id"])
    теги = клиент.get(
        f"/api/projects/{работа['id']}/tags?report={отчёт['id']}").json()
    assert теги["tags"] == []
    assert отчёт["template_name"] == ""
