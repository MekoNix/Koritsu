...
from .c_fixtures import (  # noqa: F401
    docx_байты, завести, клиент, личное_id, позвать, создать_проект, войти,
    сосед, хозяин,
)
...
# ── лента отчётов пространства ───────────────────────────────────────────────
#
# `GET /api/reports?workspace_id=` — то, из чего сделан главный экран отчётов:
# человек видит написанное им сразу, без выбора работы. Проверяется здесь не
# «маршрут отвечает», а три обещания, каждое из которых иначе ломается молча:
# отчёты собираются со всех работ и названы своей работой, чужого и удалённого
# в ленте нет, и роль спрашивается та же, что у списка одной работы.

def test_лента_собирает_отчёты_всех_работ(клиент, хозяин):
    """Две работы — одна лента, и каждый отчёт назван своей работой."""
    ws = личное_id(клиент)
    первая = создать_проект(клиент, ws, name="Курсовая", шаблон=docx_байты())
    вторая = создать_проект(клиент, ws, name="Практика", шаблон=docx_байты())
    завести_отчёт(клиент, первая["id"], name="Глава 1")
    завести_отчёт(клиент, вторая["id"], name="Дневник")

    ответ = клиент.get(f"/api/reports?workspace_id={ws}")
    assert ответ.status_code == 200, ответ.text
    лента = ответ.json()
    assert {(о["name"], о["project_name"]) for о in лента} == {
        ("Глава 1", "Курсовая"), ("Дневник", "Практика")}
    # Работа названа в самом ответе: запрашивать её имя по `project_id` на
    # каждую карточку значило бы вернуть тот самый запрос на работу, ради ухода
    # от которого лента и собрана одним маршрутом.
    assert all(о["project_id"] for о in лента)


def test_лента_отдаёт_новые_сверху(клиент, работа):
    """Лента без выбора работы читается как «что я делал последним»."""
    ws = личное_id(клиент)
    первый = завести_отчёт(клиент, работа["id"], name="Раньше")
    второй = завести_отчёт(клиент, работа["id"], name="Позже")
    лента = клиент.get(f"/api/reports?workspace_id={ws}").json()
    assert [о["id"] for о in лента] == [второй["id"], первый["id"]]


def test_в_ленте_нет_работ_без_отчётов_и_корзины(клиент, хозяин):
    """Работа без отчётов не даёт строки, удалённая — тоже."""
    ws = личное_id(клиент)
    пустая = создать_проект(клиент, ws, name="Без отчётов", шаблон=docx_байты())
    снесённая = создать_проект(клиент, ws, name="В корзине", шаблон=docx_байты())
    завести_отчёт(клиент, снесённая["id"], name="Пропавший")
    assert клиент.delete(f"/api/projects/{снесённая['id']}").status_code == 200

    лента = клиент.get(f"/api/reports?workspace_id={ws}").json()
    assert лента == []
    assert пустая["id"] not in [о["project_id"] for о in лента]


def test_лента_не_показывает_чужое_пространство(клиент, работа):
    """Отчёт лежит в личном — в списке кафедрального его нет."""
    завести_отчёт(клиент, работа["id"], name="Личный")
    чужое = клиент.post("/api/workspaces", json={"name": "кафедра"})
    assert чужое.status_code == 201, чужое.text
    лента = клиент.get(f"/api/reports?workspace_id={чужое.json()['id']}").json()
    assert лента == []


def test_лента_требует_пространство(клиент, работа):
    """Список без пространства показывал бы чужие работы при открытом своём."""
    assert клиент.get("/api/reports").status_code == 422
    ответ = клиент.get("/api/reports?workspace_id=не-uuid")
    assert ответ.status_code == 400
    assert ответ.json()["error"]["code"] == "invalid_id"


def test_ленту_читает_участник(app, клиент, работа, сосед):
    """Роль та же, что у списка одной работы: смотреть — viewer, чужому — 404."""
    ws = личное_id(клиент)
    завести_отчёт(клиент, работа["id"], name="Общий")
    позвать(app, клиент, ws, сосед, role="viewer")

    войти(app, сосед)
    лента = клиент.get(f"/api/reports?workspace_id={ws}")
    assert лента.status_code == 200, лента.text
    assert [о["name"] for о in лента.json()] == ["Общий"]

    посторонний = завести(app, клиент, "посторонний@пример.рф")
    войти(app, посторонний)
    assert клиент.get(f"/api/reports?workspace_id={ws}").status_code == 404
