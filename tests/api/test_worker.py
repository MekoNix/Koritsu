...
def test_куски_текста_подряд_ложатся_одной_строкой(app, клиент, хозяин):
    """Склейка — на стороне записи (§11, ~100 мс). Сделанная при чтении, она
...
def test_списание_копится_по_ходу(app, клиент, хозяин):
    """§7: расход наружу во внутренних единицах. Списывается по ходу, а не в
...
def test_нет_обработчика_это_failed_а_не_вечное_ожидание(app, клиент, хозяин,
    ...
    Отсутствие обработчика приходится **изображать**: к концу ночи 2 обработчик
...
def test_отмена_между_шагами_даёт_cancelled(app, клиент, хозяин):
    ...
    останавливается сам — сделанное при этом сохраняется (§11: «закрытые теги
    остаются»)."""
...
def test_второе_задание_того_же_человека_ждёт(app, клиент, хозяин):
    """Решение владельца §11: один слот на человека, остальные ждут.
...
def test_потерянное_задание_поднимается_один_раз(app, клиент, хозяин):
    ...
    Это не ретрай упавшего обработчика (§7: «повтор упавшего только вручную»):
...
def test_уборка_убирает_и_задания_и_корзину(app, клиент, хозяин, settings):
    ...
    который работает по времени, а не по запросу (§11: «раз в час»)."""
...
def test_уборка_сносит_забытые_байты_и_щадит_ждущее_задание(app, клиент, хозяин,
                                                            settings):
    """§12: уборщик `incoming/`. Байты без живого задания и старше срока — вон.
...
def test_виду_без_нужды_секрет_не_уезжает(app, клиент, хозяин):
    ...
    должен (решение главной сессии, вариант C)."""
    w = воркер(app)
...
def test_виду_с_нуждой_секрет_и_общие_ключи_уезжают_каналом(app, monkeypatch):
    ...
    # Реестр чистится ДО подмены: настоящий обработчик `fill_tag` (агент B)
    # уже зарегистрирован, а `register` на занятый вид — беда настройки, а не
...
def test_задание_ставится_из_кода_одной_строкой(app, клиент, хозяин):
    """Форма для соседей: загрузка материала (агент D) и кнопка «собрать отчёт»
    (агент B) ставят задание не через HTTP, а этой функцией."""
    with app.state.db.session_scope() as s:
