...
def test_виду_с_нуждой_секрет_уезжает_каналом(app, monkeypatch):
    """`needs_secret=True` — секрет приходит JSON на stdin, а не переменной
    окружения: окружение наследуют внуки (tesseract, LibreOffice), канал не
    наследует никто.

    Ключей поставщиков в канале нет ни одного, и это часть уговора: ключ у
    прогона свой, он лежит в базе зашифрованным и достаётся уже в подпроцессе.
    Переменная «общего ключа владельца» стоит заведённой и не уезжает никуда.
    """
    ...
    try:
        ...
        assert спец["настройки"]["secret"] == app.state.settings.secret
        assert "ключи_окружения" not in спец
