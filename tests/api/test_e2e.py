...
её взял бы разработчик, — из журнала консольного `Mailer` (§2 правил ночи: писем
не слать).
...
владельцу, а не по пришедшему. Каждое из этих «но» — чужая зона, и ни один
агент не мог закрепить его у себя.
...
# Все журналы службы. Ключ модели не должен попасть ни в один из них (§3), и
...
def завести_проект(клиент, отказы, workspace_id: str, name: str = "работа") -> dict:
    """Проект без шаблона: оркестратор строит документ с нуля (§2)."""
...
def test_весь_путь_от_письма_до_выхода(app, клиент, отказы, ящик, settings):
    ...
    # Личное пространство завёл хук регистрации (зона C), а не этот тест.
    ...
    # Путей наружу нет — только число байт (§3).
    ...
    # `202`, а не `201`: разбор чужого файла уехал в очередь (§11), и материала
...
def test_ключ_модели_не_попал_ни_в_ответ_ни_в_журнал(клиент, отказы, ящик):
    """§3: наружу — четыре последних знака, и больше ничего.
...
def test_чужое_не_видно_а_приглашённый_видит_но_не_пишет(app, клиент, отказы,
    ...
    404, а не 403, на чужой проект — решение §3: 403 сообщал бы, что проект с
...
def test_второй_файл_не_влезает_в_квоту(app, клиент, отказы, ящик, settings):
    """§2: квота на владельца. Второй файл — `413 quota_exceeded`.
...
def test_тот_же_файл_второй_раз_квоту_не_ест(app, клиент, отказы, ящик):
    """Кэш по хешу (§4): та же методичка второй раз не занимает ни байта.
...
def test_личное_пространство_не_удаляется(клиент, отказы, ящик):
    """Личное — единственное, чего человек лишиться не может (§1)."""
...
def test_служба_не_тянет_hokoku():
    ...
    разрешённых), но проверяет именно тот случай, который в эту ночь был ближе
    всего: зона C правила `orchestrator` (это можно), а `orchestrator` знает про
    `hokoku`. Служба, потянувшая движок отчётов, перестала бы собираться без
