...
Проверяется то, из чего подпакеты исходят, не перечитывая каркас.
...
# миграцию под неё. Обнаруживается это так: `tests/api/test_migrations.py`
...
def test_миграция_поднимает_пустую_базу(settings):
    ...
    Сверяется с головой цепочки, а не с `0001_root`: корень был последней
    миграцией ровно до первой таблицы. Записанное в тесте
...
def test_у_цепочки_один_корень():
    """Миграции заводятся параллельно; свести три головы `alembic merge` может
    только при общем предке. Больше одного корня — цепочка, которую
