...
2. **чужое — 404, своё без прав — 403** (§3 и подтверждение главной сессии):
...
def test_хук_стоит_в_настоящих_аккаунтах(app):
    """У живого приложения хук стоит в настоящем списке агента B."""
...
def test_editor_не_трогает_участников(app, клиент, хозяин, сосед):
    """Участники — дело владельца (§7): `editor` меняет проекты, а не людей."""
    ws = клиент.post("/api/workspaces", json={"name": "кафедра"}).json()
...
def test_require_role_вне_запроса(app, хозяин):
    """Форма для соседей (агент D): сессия и идентификаторы, без `Request`."""
...
def test_хук_молчит_без_аккаунтов(monkeypatch):
    ...
    Это не украшение: без этого зона C не собиралась бы, пока не готова зона B,
    и два агента ждали бы друг друга вместо работы.
    """
