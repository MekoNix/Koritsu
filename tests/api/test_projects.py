...
2. **путь наружу не уезжает никогда** — это проверяется регуляркой по телу
...
def test_проект_без_шаблона(клиент, хозяин, settings):
    """Без файла оркестратор строит документ с нуля.
...
# ── форма значения по типу ───────────────────────────────────────────────────

ГОДНЫЕ = [
    ("text", {"type": "text", "text": "строка"}),
    ("markdown", {"type": "markdown", "text": "# Заголовок"}),
    ("table", {"type": "table", "rows": [["a", "b"], ["1", "2"]],
               "header": True}),
    ("formula", {"type": "formula", "latex": "E = mc^2"}),
    ("toc", {"type": "toc", "levels": 2}),
]

# Каждая беда — из тех, что раньше молча ложились на том и всплывали сборкой
# отчёта: задание, деньги и пять минут ожидания ради «в таблице нет строк».
НЕГОДНЫЕ = [
    # тип не назван вовсе — значение без типа не прочтёт никто
    ({"text": "без типа"}, "body.type"),
    ({"type": "выдумка", "text": "нет такого типа"}, "body.type"),
    # таблица: строки — список списков строк, а не что попало
    ({"type": "table", "rows": "не список"}, "body.rows"),
    ({"type": "table", "rows": [["a"], ["b", "c"]]}, "body.rows"),
    ({"type": "table", "rows": [[1, 2]]}, "body.rows"),
    ({"type": "table", "header": True}, "body.rows"),
    # формула: латех строкой
    ({"type": "formula", "numbered": True}, "body.latex"),
    ({"type": "formula", "latex": 5}, "body.latex"),
    # картинка: идентификатор артефакта, а не путь и не байты
    ({"type": "image", "caption": "Рисунок"}, "body.artifact"),
    ({"type": "image", "artifact": "../../etc/passwd"}, "body.artifact"),
    # схема: ровно одно из `artifact` и `xml`
    ({"type": "diagram", "caption": "Схема"}, "body"),
    # опечатка в имени поля не должна проглатываться: «captionn» собрался бы
    # без подписи, и объяснить это было бы нечем
    ({"type": "text", "text": "а", "textt": "б"}, "body.textt"),
]


def test_годное_значение_принимается_по_каждому_типу(клиент, хозяин):
    """Проверка формы не должна мешать работать: годное едет как ехало."""
    ws = личное_id(клиент)
    pid = создать_проект(клиент, ws, шаблон=docx_байты())["id"]
    for имя, значение in ГОДНЫЕ:
        ответ = клиент.put(f"/api/projects/{pid}/values/цель", json=значение)
        assert ответ.status_code == 200, f"{имя}: {ответ.text}"


def test_кривое_значение_отвергается_с_местом(клиент, хозяин):
    """`invalid_value` и `where` на поле, которое не так.

    `where` здесь — не украшение: по нему интерфейс подписывает строку в
    редакторе значения, а без него человеку сообщается «что-то не то» о JSON в
    десять строк.
    """
    ws = личное_id(клиент)
    pid = создать_проект(клиент, ws, шаблон=docx_байты())["id"]
    for значение, место in НЕГОДНЫЕ:
        ответ = клиент.put(f"/api/projects/{pid}/values/цель", json=значение)
        assert ответ.status_code == 400, f"{значение}: {ответ.text}"
        беда = ответ.json()["error"]
        assert беда["code"] == "invalid_value", значение
        assert беда["where"] == место, f"{значение}: {беда}"
        # Наружу — по-английски и без куска русской беды `wire`.
        assert беда["message"].isascii(), беда


def test_кривое_значение_не_заводит_версию(клиент, хозяин):
    """Отказ обязан быть до записи: иначе история тега копит мусор, который
    нельзя ни собрать, ни откатить."""
    ws = личное_id(клиент)
    pid = создать_проект(клиент, ws, шаблон=docx_байты())["id"]
    assert клиент.put(f"/api/projects/{pid}/values/цель",
                      json={"type": "table", "rows": "нет"}).status_code == 400
    теги = {т["key"]: т for т in
            клиент.get(f"/api/projects/{pid}/tags").json()["tags"]}
    assert теги["цель"]["filled"] is False


def test_ссылка_на_несуществующий_артефакт_отвергается(клиент, хозяин):
    """Картинка ссылается на артефакт, которого нет, — это отказ, а не отчёт с
    дырой на месте рисунка."""
    ws = личное_id(клиент)
    pid = создать_проект(клиент, ws, шаблон=docx_байты())["id"]
    ответ = клиент.put(f"/api/projects/{pid}/values/цель",
                       json={"type": "image", "artifact": "0123456789abcdef"})
    assert ответ.status_code == 400, ответ.text
    assert ответ.json()["error"]["code"] == "invalid_value"


def test_объявленный_тип_тега_не_сверяется(клиент, хозяин):
    """Тип в манифесте бывает угадан по метке, и защищать догадку от правды
    служба не должна: расхождение показывает проверка перед сборкой."""
    ws = личное_id(клиент)
    pid = создать_проект(клиент, ws,
                         шаблон=docx_байты(("схема", "выводы")))["id"]
    теги = {т["key"]: т for т in
            клиент.get(f"/api/projects/{pid}/tags").json()["tags"]}
    assert теги["схема"]["type"] == "diagram", "иначе тест ничего не проверяет"
    ответ = клиент.put(f"/api/projects/{pid}/values/схема",
                       json={"type": "markdown", "text": "пока словами"})
    assert ответ.status_code == 200, ответ.text
...
def test_корзина_каталог_на_месте_восстановление(app, клиент, хозяин, settings):
    """Удаление — это метка в базе, а не `rm -rf`: десять дней передумать."""
    ws = личное_id(клиент)
    проект = создать_проект(клиент, ws, шаблон=docx_байты())
    pid = проект["id"]
...
def test_bytes_used_считает_по_владельцу(app, клиент, хозяин, сосед, settings):
    """Экспорт для соседей: сумма каталогов проектов **создателя**.
