...
def test_том_обязателен_умолчания_нет():
    """Решение владельца К.2: путь задаётся снаружи. Умолчание вроде `./data`
...
def test_умолчания_те_что_решил_владелец():
    ...
    assert s.env == "dev"
    assert s.user_quota_bytes == 250 * MB          # §2
    assert s.file_max_bytes == 10 * MB             # §2
    assert s.session_days == 30                    # §7
    assert s.trash_days == 10                      # §2
    assert s.registrations_per_ip_per_day > 0      # §7
...
# ── прокси и адрес сайта (правка агента B, ночь 1) ───────────────────────────
...
#
# Таймаут и потолок памяти подпроцесса разбора (§3). Числа были константами в
# `materials/parsing.py`, пока `settings.py` правился каркасом; после сведения
# они здесь — и проверяется тут ровно то, ради чего их сюда перенесли: машина,
...
def test_очередь_умолчания():
    """Слоты — решение владельца §11 дословно: «2 тяжёлых задания на машину,
    1 на пользователя, остальные ждут в очереди»."""
    s = Settings.from_env(окружение())
