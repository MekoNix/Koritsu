...
базе, — худший случай для прокси (решение владельца §7), потому что он остаётся
...
def test_внешний_вход_заведён_с_первого_дня(client):
    """§5: `/api/v1/` версионируется и документируется наружу, поэтому версия
...
def test_вход_сайта_отвечает_на_несуществующее(client):
    ...
    Адрес намеренно ничей: `/api/projects` и `/api/workspaces` завёл агент C, и
    проверять на них «маршрута нет» значило бы проверять, что зона C ещё не
    написана. Проверяем то, что и проверяли: вход `/api` есть, и незнакомый
...
def test_описание_апи_собирается(client):
    """OpenAPI (§5) — из него генерируется клиент сайта; если он не собирается,
...
def test_журнал_не_пишет_ни_строки_запроса_ни_заголовков(client, caplog):
    """Решение владельца §3: содержимое тел не пишется никогда. Строка запроса
