...
Писем нет, поэтому эта таблица — единственное, что переживает закрытую
...
def test_отмена_ждущего_задания_тоже_уведомляет(app, клиент, хозяин):
    """Уведомление приходит и при отмене ждущего. Воркер такое задание не увидит
...
def test_потерянное_дважды_задание_уведомляет(app, клиент, хозяин):
    """Уведомление приходит и при `worker_lost`. Воркер, которого не стало, ничего
...
def test_уведомление_о_выгрузке_несёт_ссылку_на_архив(app, клиент, хозяин):
    """Скачивание из уведомления: идентификатор архива
...
def test_чужое_уведомление_это_404_а_не_403(app, клиент, хозяин, сосед):
    """`403` сообщал бы, что строка с таким идентификатором есть."""
    поставить(клиент)
    прогнать(app)
    nid = клиент.get("/api/notifications").json()["notifications"][0]["id"]
