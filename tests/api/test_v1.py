...
Здесь проверяется ровно то, ради чего вход заведён отдельным (§1, §11):
...
* **аккаунты, ключи и админка ключом не открываются** (§11);
...
def test_задания_видны_ключом(app, клиент, связка):
    """§11 обещает внешнему клиенту задания, а не только проекты."""
...
def test_загрузка_просит_своё_право(app, клиент, хозяин):
    ...
    ключи (§11: `materials:write` отдельным правом)."""
...
def test_право_объявленное_в_маршруте_старше_умолчания(app, клиент, хозяин):
    ...
    Он и проверяется здесь, потому что им пользуются агенты B и D: объявленное
...
def test_аккаунты_ключи_и_админка_ключом_не_открываются(app, клиент, хозяин,
                                                        путь):
    """§11: «Аккаунты и ключи моделей через токен не даём».
...
def test_шестьдесят_запросов_в_минуту(app, клиент, связка):
    """Потолок на ключ (§11). Считается в памяти процесса — см. `tokens`."""
...
def test_вход_описан_отдельным_тегом(app):
    """§5: наружу документируется и версионируется только `/api/v1`, значит у
