...
Продолжение `test_e2e.py` и по той же причине. Каждый кусок ночи 2 проверен
своим тестом: очередь — `test_jobs`, прогоны — `test_runs`, потоки —
`test_sse`, ключи — `test_v1`, выгрузка — `test_export`. Ломается же не кусок, а
стык между кусками, и стыков этой ночью прибавилось разом восемь:
...
* откат тега поверх значения, записанного моделью, — это две зоны в одной
  строке;
...
события. Подделан ровно один шов — провод к модели (`b_fixtures`): сети ночью
нет, а всё, что за проводом, — лестница, лимит, журнал, склейка текста — своё.
...
def test_путь_второго_этапа_целиком(app, клиент, отказы, caplog, модель):
    ...
    # Материала ещё нет — и это не задержка, а договор: §11 говорит «один путь,
    # через очередь», и синхронной ветки для маленьких файлов не осталось.
...
def test_расход_один_и_тот_же_в_usage_и_в_админке(app, клиент, отказы, caplog):
    ...
    её договор. До сведения ночи 2 сумм было две, написанных порознь: одна в
...
def test_исчерпанный_лимит_плана_отказывает_402(app, клиент, отказы, caplog,
    ...
    Потолок приходит настройкой (`KORITSU_FREE_MONTHLY_UNITS`, §11), а не
...
def test_все_отказы_второго_этапа_одной_формы(app, клиент, отказы, caplog,
    ...
    Отказы этой ночи приходят из шести зон сразу — очередь, прогоны, ключи,
    админка, CSRF, внешний вход, — и в каждой свой соблазн ответить по-своему.
    """
    from api.errors import ErrorOut

    test_путь_второго_этапа_целиком(app, клиент, отказы, caplog, модель)

    # Добавим к собранному то, что сценарий не проходит: беды каждой зоны ночи.
