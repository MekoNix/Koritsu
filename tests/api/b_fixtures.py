"""
Оснастка зоны B (прогоны, лимиты, потоки событий, уведомления, версии).

Отдельным модулем, как `c_fixtures.py`, и по той же причине: общий conftest
правится четырьмя агентами сразу, а фикстуры зоны нужны только своим тестам.
