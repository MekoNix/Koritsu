...
1. **чужое задание — 404, а не 403.** Разные ответы на «нет такого» и «не твоё»
   превращают перебор идентификаторов в способ узнать, какие из них заняты (§3);
...
3. **лимит на человека — свойство захвата, а не постановки.** Второе задание
   ложится в очередь и ждёт (§11: «остальные ждут»), а не получает отказ.
"""
...
def test_карточка_не_рассказывает_про_воркер(клиент, хозяин):
    ...
    поля покажет админка (агент C), у неё другой договор.
    """
...
def test_второе_задание_ложится_в_очередь_а_не_в_отказ(клиент, хозяин):
    """Решение владельца §11: «остальные ждут в очереди».
...
def test_карточка_двигает_срок_хранения(app, клиент, хозяин):
    """§7: «90 дней с последнего обращения». Значит, обращение обязано двигать
...
def test_отмена_работающего_ставит_флаг(app, клиент, хозяин):
    """Работающее не убивается сразу: §11 обещает «закрытые теги остаются», а
...
def test_уборка_сносит_старые_с_событиями_и_не_трогает_свежие(app, клиент,
                                                              хозяин, settings):
    """§7: «90 дней с последнего обращения; потом удаляются и запись задания, и
    её артефакты». Считается `last_seen_at`, а не конец работы."""
