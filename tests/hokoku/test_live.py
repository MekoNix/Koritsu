...
Проверяется не «функции не падают», а четыре обещания записки
`ref/koritsu-hokoku-live-2026-09-03.md`, каждое из которых стоит дефекта:

  1. операции чистые — прежний список цел, значит отмена есть каждый ход (§В.1);
  2. документ это сборка списка, и собирает её тот же `render` — подписи, счётчики
     SEQ и поля REF работают, а не «должны бы» (§В.3);
  3. `validate_work` ловит дубли ключей и ссылки в никуда **до** сборки, потому что
     после неё видно только глазами и только «?» посреди отчёта (§Д.3);
  4. схемы инструментов годятся строгому режиму поставщика: у каждого поля есть тип,
     `additionalProperties` закрыт, объединение помечено `type`/`const` (§Г).
"""
import io
...
def test_валидатор_ловит_ссылки_в_никуда(png):
    ...
    # Ошибка, а не предупреждение (решение владельца 2026-09-04): цель ссылки в
...
def test_ссылка_на_ненумерованный_блок_это_ошибка_а_не_предупреждение():
    ...
    Уровень — `error` (решение владельца 2026-09-04): «?» в документе хуже отказа,
...
def test_места_под_текст_знают_соседей(png):
    """Решение владельца 2026-09-04: скелет петлёй, весь связный текст — одним проходом
    по готовому списку, видя соседей."""
