...
def test_замечание_о_ссылке_называет_блок_словами_человека(png):
    """Замечание читает человек, а `b-02` ему ничего не говорит: раздел он ищет
    глазами по названию. Ключ при этом остаётся — им адресуется правка."""
    w = hokoku.Work((live.block("b-01", Image(png, caption="Схема")),
                     live.block("b-02", Markdown("см. {ref:b-09}"), label="Реализация")))
    беда = hokoku.validate_work(w)[0]
    assert "«Реализация» (b-02)" in беда.message      # откуда ссылка
    assert "«b-09»" in беда.message                   # куда она вела
    assert "«?»" in беда.message                      # и что будет в документе


def test_ссылку_в_никуда_можно_снять_и_собрать_работу(png):
    """Отказ собрать работу стоит человеку целого прогона, а абзац без номера
    рисунка он поправит глазами. Текст при этом остаётся целым."""
    w = hokoku.Work((live.block("b-01", Image(png, caption="Схема")),
                     live.block("b-02", Markdown("см. {ref:b-01} и {ref:b-09} дальше")),
                     live.block("b-03", Table([["a"]], caption="Замеры {ref:b-09}"))))
    без = hokoku.strip_refs(w, ["b-09"])
    assert без.get("b-02").value.text == "см. {ref:b-01} и дальше"   # ссылка на своё цела
    assert без.get("b-03").value.caption == "Замеры"
    assert hokoku.validate_work(без) == [] and w.get("b-02").value.text.count("{ref:") == 2
    assert "?" not in _texts(hokoku.assemble(без))[1]
...
def test_инструменты_чистые_и_зовутся_по_имени(png):
    ...
    w, ответ = hokoku.call_tool(w, "insert_block", {
        ...
        "label": "Постановка"})
    # Ключ не назвали — его выдаёт служба и называет в ответе: молча подменённый
    # адрес хуже отказа, по нему потом ставят `{ref:}`.
    assert ответ == {"key": "b-01", "kind": "heading", "blocks": 1,
                     "note": "ключ выдан службой: b-01 — им и адресуй этот блок"}
