...
СТРОЕНИЕ = {
    ...
    # Вид раздела (`kind`) модель называет своими словами по условию, тип
    # (`type`) берёт из перечня движка: два поля, а не одно, потому что тип,
    # угаданный по имени раздела, даёт схему, про которую написано прозой.
    "sections": [
        ...
        {"key": "схема", "title": "Схема алгоритма", "kind": "схема",
         "type": "diagram", "prompt": "блок-схема сортировки"},
        {"key": "замеры", "title": "Замеры", "kind": "замеры",
...
def test_условие_сканом_останавливает_работу_на_показ(tmp_path, сценарий,
                                                      fake_tesseract):
    """Распознанное показывается ДО решения.
