...
def test_режимы_спрашиваются_у_строителя_а_не_пишутся_списком():
    ...
    Единственное, чего в перечне нет, — псевдонимы (решение владельца §12:
    «наружу только `gost_19_701_90`»): два имени одного режима — это два пункта
...
def test_неизвестный_язык_отказ_с_кодом_а_не_KeyError():
    ...
    # Текст английский: его читает клиент службы (§7).
