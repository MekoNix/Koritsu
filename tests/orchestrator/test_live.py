...
def test_агент_собирает_скелет_и_список_меняется(project, live_endpoint):
    """Цепочка инструментов: заголовок, схема из исходника студента, схема блоком.
...
def test_ход_работы_виден_модели_до_первого_вызова(project, live_endpoint):
    ...
    И он же едет НЕДОВЕРЕННЫМ куском — его писала модель по файлам студента.
    """
    ...
    # Файл студента с попыткой инъекции по-прежнему едет своим куском в рамке.
