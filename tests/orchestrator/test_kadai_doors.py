...
def test_строение_превращается_в_заготовки_блоков(project, endpoint):
    ...
    # Заготовка раздела, который заполняет инструмент, несёт вид в самой
    # пометке: иначе она неотличима от места под текст, проход текста заливает
    # её прозой, и пропажу листинга человек находит в собранном отчёте.
    assert блоки[3]["value"]["text"].startswith(hokoku.live.draft_mark("code"))
    assert hokoku.live.draft_kind(блоки[3]["value"]["text"]) == "code"
    assert "Листинг программы" in блоки[3]["value"]["text"]
...
def test_готовое_строение_модели_не_стоит(project, endpoint):
    ...
    # стоит черновик с видом в пометке, который петля заменит настоящей таблицей.
    assert блоки[0]["value"]["text"] == "## Ход работы"
    assert hokoku.live.draft_kind(блоки[1]["value"]["text"]) == "table"
    assert "Ход работы" in блоки[1]["value"]["text"]
...
def test_условие_сканом_читается_ocr_и_доступно_человеку(project, fake_tesseract):
    """Скан читается OCR, распознанное показывают.
