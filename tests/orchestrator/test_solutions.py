...
def test_условие_всегда_в_папке_контекста(project):
    ...
    Общий файл, названный условием, едет в промпт и без галочки: подключение
    отвечает на вопрос «показать ли заодно», а условие — это то, что решают.
    """
...
def test_общие_файлы_работы_едут_в_промпт_после_подключения(project):
    """Методичка кафедры доезжает до задачи тогда, когда её к задаче подключили.

    Общий файл лежит на работе один раз и виден любому её решению, но нужен он
    не каждой задаче: за лишний кусок промпта платит человек, а чужая методичка
    сбивает модель так же, как чужое условие. Поэтому по умолчанию не едет
    никуда, а подключается решению поштучно.
    """
    ...
    assert первое.context_ids() == {свой.id}
    assert второе.context_ids() == set()

    assert первое.set_included_common([общий.id]) == [общий.id]
    assert первое.context_ids() == {свой.id, общий.id}
    # Подключение хранится у решения: соседнее его не видит.
    assert второе.context_ids() == set()


def test_подключённый_общий_файл_снимается_тем_же_полем(project):
    """Список заменяется целиком: галочка снимается там же, где ставилась."""
    общий = project.store().add(b"chuzhaya tema\n", name="t.txt", do_ocr=False)
    решение = project.create_solution(ПЕРВОЕ)

    решение.set_included_common([общий.id])
    assert решение.context_ids() == {общий.id}
    assert решение.set_included_common([]) == []
    assert решение.context_ids() == set()


def test_общий_файл_положенный_позже_сам_не_едет(project):
    """Хранится подключённое, а не снятое: завтрашний файл в промпт не попадает.

    Иначе файл, положенный в работу ради соседней задачи, молча удлинил бы
    промпт этой — и человек увидел бы это только по счёту.
    """
    старый = project.store().add(b"staryj\n", name="s.txt", do_ocr=False)
    решение = project.create_solution(ПЕРВОЕ)
    решение.set_included_common([старый.id])

    новый = project.store().add(b"novyj\n", name="n.txt", do_ocr=False)
    assert решение.context_ids() == {старый.id}
    assert решение.included_common() == [старый.id]


def test_решение_прежней_формы_помнит_снятое(project):
    """Запись, заведённая до правила, читается как раньше: снятое — снято.

    Человек однажды выбрал, что показывать; менять этот выбор задним числом
    нельзя — прогон стоил бы других денег и дал бы другую работу.
    """
    общий = project.store().add(b"ustav\n", name="u.txt", do_ocr=False)
    снятый = project.store().add(b"chuzhoj\n", name="c.txt", do_ocr=False)
    решение = project.create_solution(ПЕРВОЕ)
    решение.put_state("kadai-общие", {"excluded": [снятый.id]})

    подключены = set(решение.included_common())
    assert общий.id in подключены
    assert снятый.id not in подключены
    assert снятый.id not in решение.context_ids()


def test_общие_файлы_подключаются_к_решению_а_не_к_работе(project):
    """У работы целиком подключать нечего: общие файлы — это её собственные файлы."""
    with pytest.raises(OrchestratorError):
        project.set_included_common([])
