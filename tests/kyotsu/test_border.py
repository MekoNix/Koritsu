...
ОБЩИЙ = "kyotsu"
# Листья: предметные пакеты, которые друг о друге не знают. `kokuban` (доска)
# среди них самый строгий: у него нет даже зависимости от общего пакета, и своё
# правило — «ни диска, ни модели» — он проверяет отдельно
# (`tests/kokuban/test_border.py`). Здесь он нужен по другой причине: пока имя
# не названо, `import kokuban` в службе не поймал бы никто.
ЛИСТЬЯ = ("hokoku", "llm", "materials", "fragmos", "uml_generator", "zuhyo", "kokuban")
...
def test_службе_разрешены_только_четверо():
    """Пятый импорт сюда попадёт не по недосмотру, а мимо принятого решения.
