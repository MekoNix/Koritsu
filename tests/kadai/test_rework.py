...
def test_замечание_к_условию_перечитывает_само_условие(doors):
    """Условие правится текстом на месте, а работа помнит то, которое прочитала в
    прошлый раз. Не перечитав его, переделка «по новому условию» пошла бы по
    старому — и это была бы работа, за которую человек заплатил дважды."""
    session = прогнать(doors)
    doors.project.set_condition_text("Написать программу поиска и отчёт.")
    итог = rework.apply(session, note="условие я понял иначе", kind="условие")

    assert итог["stages"][0] == "приём"
    assert "начиная с чтения условия" in итог["note"]
    assert session.work.condition_text == "Написать программу поиска и отчёт."
    assert "Написать программу поиска" in doors.asked[-1]["data"][0][1]
