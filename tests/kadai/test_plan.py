...
Стерегутся три вещи. Первая: умолчание — проход без остановок, и никакая
просьба не появляется сама. Вторая: остановка
...
def test_план_знает_какие_стадии_нужны_этому_виду_работы(profile):
    """Строение без стадии «решение» — это данные, а не ветка кода."""
    записка = kadai.parse({"name": "записка",
                           "stages": ["приём", "разбор задания", "шаблон", "тексты",
                                      "сборка", "архив"],
                           "kinds": {"введение": {"required": True}}})
    plan = kadai.plan_of(записка)
