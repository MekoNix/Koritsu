...
def test_состав_архива_и_источники_без_путей():
    ...
    assert names == ["отчёт.docx", "отчёт.pdf", "шаблон.docx", "исходники/sort.py",
                     "схемы/схема_алгоритма.drawio", "решение.md", ".metadata"]
...
def test_архив_на_сдачу_несёт_только_работу():
    """Второй архив — то, что сдают: отчёт, `исходники/` и `схемы/`.

    Бланк, `решение.md` и запись о сборке в него не кладутся: их принимали за
    часть работы и несли вместе с ней. Запись о сборке для него и не требуется —
    она лежит в полном архиве, собранном рядом, и человек её не теряет.
    """
    entries = kadai.plan_archive(light=True, report_artifact="d0cx",
                                 pdf_artifact="pdf1", template_artifact="aa11",
                                 source_texts=[("sort.py", "def sort(a): ...")],
                                 diagram_texts=[("b-10", "<mxfile/>")],
                                 solution=kadai.solution_md([("Введение", "Текст.")]))
    assert [e.name for e in entries] == ["отчёт.docx", "отчёт.pdf",
                                         "исходники/sort.py", "схемы/b-10.drawio"]
...
def test_записка_рассказывает_как_собрано():
    """Запись о сборке отвечает на вопрос «откуда это взялось» — словами человека.

    Условие в ней дословно: работа отвечает на задачу, и без её текста рядом
    сверить ответ не с чем. Внутренних имён — имён модулей, кодов замечаний,
    названий инструментов — в ней нет: читает её человек, а не служба, и
    поломкой они выглядят именно у него.
    """
    текст = kadai.notice_text(work_id="w-1", profile="учёт продажи носков", wishes="покороче",
                              condition="Посчитать остатки по складам.",
                              stages=[{"name": "приём", "state": "сделано", "note": "18 файлов"}],
                              spent={"units": 41200.0, "cap": 200000.0, "share": 0.21,
                                     "estimated_share": 0.04},
                              versions={"введение": 3},
                              problems=[kadai.problem("нет_кода", "листингов в работе нет",
                                                      level="warning")])
    assert "покороче" in текст and "введение: v3" in текст
    assert "оценено, а не измерено: 4%" in текст
    assert "Посчитать остатки по складам." in текст
    # Замечание названо словами, а его код остался интерфейсу.
    assert "Предупреждение: листингов в работе нет" in текст
    for внутреннее in ("нет_кода", "hokoku", "kadai", "orchestrator", "LibreOffice"):
        assert внутреннее not in текст
