...
от того, кто работу примет. Второе: отказ обязан называть пакет, адрес правки
и подпись — иначе
...
def test_отказ_шва_печатает_подпись_которую_ждём():
    ...
    assert "make_template" in текст and "hokoku" in текст and "блоки" in текст
...
def test_решение_названо_сведённым_вместе_с_put_source():
    ...
    (`orchestrator.live`), и шов обязан называть вслух это.
    """
...
def test_разбор_условия_требует_отделимости_распознанного():
    """Распознанное OCR показывается человеку. Пока оно
...
def test_проверка_кода_названа_сведённой_и_называет_дверь():
    """Шов «проверка кода» держит правило: код проверяется статически.
