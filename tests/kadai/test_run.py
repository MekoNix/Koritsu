...
Здесь стерегутся решения владельца 2026-09-04, каждое из которых иначе тихо
превращается в свою противоположность:

* **вид работы не фиксирован** — профиль сочиняется по условию, а `kursovaya.yaml`
  остаётся палитрой и умолчанием; работа без кода и схем теряет стадию
  «решение», а не проходит её вхолостую;
...
def test_профиль_сочиняется_из_условия_а_не_берётся_из_папки(doors):
    """Умолчание — образец формы, а не приговор: имя вида работы приходит из
    ответа модели, а обязательные разделы — из того, что она объявила."""
    ...
    # Титульника и оглавления в сочинённом строении нет — и профиль их не
    # требует, хотя в файле-умолчании они обязательны.
    assert "титульник" not in обязательные


def test_работе_без_кода_и_схем_стадия_решения_не_нужна(doors):
    """У реферата петле производить нечего, и показать стадию сделанной значило
    бы соврать полоской хода о работе, которой не было."""
...
def test_условие_сканом_кладётся_в_снимок_и_останавливает_прогон():
    ...
    project = FakeProject()
    project.add_material("Дано: массив из N чисел.".encode("utf-8"), "методичка.png",
                         condition=True)
...
def test_текст_пишется_одним_проходом(doors):
    ...
    решение владельца 2026-09-04, и здесь оно проверяется счётом вызовов."""
    прогнать(doors)
...
def test_в_архиве_всё_по_описи_и_пометка_про_незапущенный_код(doors):
    ...
    assert "Вид работы: курсовая" in записка and "Расход:" in записка
...
def test_работа_переживает_перезагрузку_процесса(doors):
    ...
    assert заново.plan.profile.name == "курсовая"
...
def test_условие_и_пожелания_едут_данными_а_не_вопросом(doors):
    ...
    данные. В файле студента бывает написано «забудь предыдущие указания»."""
    session = kadai.run.new(doors.services(),
...
def test_граница_until_держится_даже_если_стадия_пропущена(doors):
    ...
    или пропустили."""
    doors.строение = {**doors.строение, "work_kind": "реферат",
                      "expects": {"code": False, "tables": False, "diagrams": False}}
    session = kadai.run.new(doors.services())
