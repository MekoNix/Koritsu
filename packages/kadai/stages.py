...
удобно рисовать полоску. Отсюда семь, а не три и не двенадцать: после стадии
«шаблон» всё дорогое уже завязано на понимание условия и на структуру, и
исправлять их после стадии «тексты» значит выбрасывать оплаченное.

Что здесь важно и неочевидно.

**Пропущенная стадия — отдельное состояние.** У работы, где производить нечего,
не будет «решения»: ей не нужны ни код, ни таблицы, ни схемы. Показать её как мгновенно прошедшую значило бы
...
приходит в момент завершения стадии. Оба ведут в одно состояние `waiting_user`
через один вызов. Но снимок обязан говорить, ЧТО именно показывают: без этого
ожидание неотличимо от зависшей работы, и человек нажмёт «отменить» — то есть
выбросит оплаченное.
...
# Состояния работы целиком — ключи API, поэтому латиницей.
...
def step(work: Work, name: str, *, current: str = "", done: int | None = None,
    ...
    неизвестного, и рисовать её нельзя.
    """
    st = work.stage(name)
    if st.state != RUNNING:
