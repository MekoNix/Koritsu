...
**Вид работы не фиксирован, и вида работы в коде нет вовсе** — это несущее
решение. Служба обрабатывает любой документ агентом и своими инструментами:
чем окажется работа — учебной, отчётом о продажах, запиской, — знает только
условие. Поэтому файла-профиля здесь больше нет: ни папки `profiles/`, ни
загрузчика, ни умолчания. Профиль этой работы складывается **целиком из
ответа модели** (`compose`) по условию и пожеланиям, а всё, что осталось в
коде, — утверждения не про вид работы, а про движок отчётов: перечень типов
содержимого, запреты и верхняя граница числа разделов.
...
**Обязательные разделы называет модель.** Что работа обязана содержать, знает
условие, а не мы: `required_kinds` в ответе — это её собственное утверждение, и
сито «нет обязательного» проверяет структуру против него. Плата названа вслух:
поймать «условие поняли неверно» этим ситом нельзя — модель сверяется сама с
собой. Для этого есть отдельная стадия разбора задания с показом человеку, и
она стоит раньше.

**Нижней границы объёма нет.** «Работы короче шести разделов не бывает» —
утверждение про один вид работы, и применить его ко всякому значило бы
отвергнуть законное строение. Верхняя граница остаётся (`MAX_SECTIONS`): она не
про вид работы, а про движок и про деньги — каждый раздел это место под текст в
одном ответе прохода текстов.
...
# под текст в одном ответе модели. Нижней границы нет.
MAX_SECTIONS = 18
...
def check_structure(profile: Profile, structure) -> list:
    ...
    Первое из четырёх сит и самое дорогое по цене пропущенной беды.
