...
**Ключ раздела — имя, а не адрес, и потому его форма не отказ.** Адреса блокам
выдаёт служба (`hokoku.live.new_key`), придуманный моделью ключ тегом не
становится никогда, и ронять оплаченное строение из-за пробела в названии
раздела не за что. Замечанием о нём сказано: по ключу человек ищет раздел в
замечаниях, и два ключа, различающиеся только нормализацией NFC (сила —
`hokoku.tags.norm_key`), он прочтёт как один раздел. Знаки, которых тег не
терпит, перечислены здесь копией (`BAD_KEY_CHARS`, сила — `hokoku.tags.TAG_RE`)
и помечены как копия: дверь `orchestrator.norm_key` убрала бы её.
...
# Насколько близкими считаются два имени вида раздела. То же число, что у
# подсказки «похоже на» (`errors.hint`), и не случайно: механизм сравнения один,
# и второй порог рядом означал бы, что «похоже» в подсказке и «похоже» при
# сопоставлении — разные слова. Ниже него начинаются пары вроде «Вывод» и
# «Таблица действий», где притянутое сходство было бы враньём.
БЛИЗОСТЬ = 0.6
...
def compose(answer: dict, *, stages, problems=None) -> Profile:
    ...
    **Обязательный вид, которого среди разделов нет, — не отказ.** Модель
    называет один и тот же раздел дважды: `sections` она пишет заголовками
    («Таблица действий»), а `required_kinds` — своими словами о том же
    («План действий»). Строгая сверка роняла на этом целый прогон, то есть
    человек платил за решение задачи и получал ошибку про имена полей. Поэтому
    несовпавшие имена сопоставляются по близости тем же механизмом, каким
    служба подсказывает опечатки, а несопоставленное отбрасывается с
    замечанием уровня `warning` — оно уезжает в `problems`, если его дали.

    Замечание уровня, а не текста: показывать человеку «обязательными объявлены
    виды разделов, которых в строении нет» незачем — это разговор модели с
    самой собой, и чинить его человеку нечем.
    """
    ...
    заявлены = [norm_key(k) for k in (answer.get("required_kinds") or ())]
    имена = _kinds_of(answer.get("sections") or (), ())
    обязательны, потеряны = _match_required(заявлены, имена)
    kinds = _kinds_of(answer.get("sections") or (), обязательны)
    if потеряны and problems is not None:
        problems.append(problem(
            "обязательный_вид_без_раздела",
            "модель объявила обязательными виды разделов, которых в строении нет: "
            + ", ".join(f'"{k}"' for k in потеряны)
            + ". Они отброшены: работа собирается по тем разделам, которые в "
              "строении есть",
            level="warning"))
...
def _match_required(заявлены, имена) -> tuple[list, list]:
    """Заявленные обязательными виды → (имена разделов, несопоставленное).

    Сопоставление по близости — то же `difflib`, которым служба подсказывает
    «похоже на»: заявленное имя ищется среди имён разделов без учёта регистра, и
    найденное считается обязательным **под именем раздела** — виды в строении
    адресуются именами разделов, и записать обязательность под чужим именем
    значило бы объявить обязательным вид, которого в работе нет.

    Одно имя раздела на одно заявленное: два заявленных, притянутых к одному
    разделу, — это по-прежнему один обязательный раздел, и второй раз он в
    список не попадает.
    """
    по_нижнему = {str(имя).lower(): имя for имя in имена}
    вышло: list = []
    потеряны: list = []
    for заявлено in заявлены:
        if заявлено in имена:
            подходит = заявлено
        else:
            близкие = difflib.get_close_matches(str(заявлено).lower(),
                                                list(по_нижнему), n=1, cutoff=БЛИЗОСТЬ)
            подходит = по_нижнему[близкие[0]] if близкие else ""
        if not подходит:
            потеряны.append(заявлено)
        elif подходит not in вышло:
            вышло.append(подходит)
    return вышло, потеряны
...
def _check_key(key, i: int, seen: dict) -> list:
    """Ключ раздела: непустой; форма и повтор — замечание, а не отказ.

    Разница в силе не описка. Ключ раздела — **имя раздела в строении, а не
    адрес блока**: адреса блокам выдаёт служба (`hokoku.live.new_key`), и ключ,
    придуманный моделью, тегом не становится никогда. Значит пробел или
    двоеточие в нём ничего не ломают, а уронить об них целую стадию — это
    выбросить оплаченное строение работы из-за названия раздела.

    Сказать о нём всё равно надо: по ключу человек ищет раздел в замечаниях, и
    два ключа, различающиеся только нормализацией, он прочтёт как один раздел.

    Пустой ключ остаётся отказом: раздел без имени нечем назвать ни в
    замечании, ни в вопросе к человеку.
    """
    out = []
    ...
    if bad:
        out.append(problem("ключ_не_тег", f'ключ раздела "{key}" содержит '
                                          f"{', '.join(repr(c) for c in bad)}: именем "
                                          "он годится, а адресом блока не станет — "
                                          "адреса блокам выдаёт служба сама",
                           key=key, level="warning"))
    if norm in seen:
        out.append(problem("ключ_повторён", f'ключ раздела "{key}" совпадает с '
                                            f'"{seen[norm]}" после нормализации: в '
                                            "замечаниях эти разделы будет не различить",
                           key=key, level="warning"))
    else:
        seen[norm] = key
    return out
