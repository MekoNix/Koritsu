...
Половина сценария сегодня стоит на том, чего у соседей нет. Здесь это названо
поимённо и записано данными, чтобы отказ был не словом «потом», а адресом:
пакет, место в коде, подпись ожидаемой функции и одна строка про то, что
сломается без неё.

**Ни одного `import` службы.** Двери приходят аргументом — `Services`, — а не
импортом, и это не стилистика. Правило импорта (`orchestrator/__init__.py`:
«из него не импортирует никто») слоя над оркестратором не предусматривает
вовсе; поправка на порядок слоёв — открытая развилка, и она не закрыта. Пока
она не закрыта, `import orchestrator` в `kadai` был бы нарушением действующего
правила. Аргумент вместо импорта закрывает вопрос: правило соблюдено дословно,
а поправку можно принять или отвергнуть, не трогая этот пакет. Движок отчётов
— исключение (правило разреза 2.0.0a4.3 разрешает `hokoku` чистыми функциями),
и живёт оно в одном файле `blocks.py`, а не расползается по швам.
...
Как отказывает шов: `NotReady` с текстом «жду `X` от `Y` (адрес): подпись» и
...
Состояние на версию 2.0.0a4.3: сведены все швы, кроме двух, и оба недоведённых
названы здесь поимённо: **разбор условия** (распознанное OCR отделимо только
целым материалом — человеку показывается весь текст условия с пометкой, а не
распознанные куски) и **отпечаток входа** (второго `prompt_hash` по стабильным
кускам нет, поэтому «этот блок устарел» от «был другой прогон» не отличается). Ни
один из двух не отказывает вызовом: они не двери, а точность — работа идёт,
просто грубее, чем задумано. Всё остальное зовётся по-настоящему.
"""
...
class Seam:
    ...
    спрашивать. С ней ожидание и код называют одно и то же одними словами.
    """

    name: str
    neighbor: str            # чей это пакет
    address: str             # место в коде соседа
...
SEAMS: dict[str, Seam] = {
    ...
    "шаблон": Seam(
        ...
        address="orchestrator.doors.make_template, hokoku.live (блоки "
                "вместо тегов). Сведено",
        ...
        awaits="шаблона как документа больше нет вовсе: работа — список блоков, и "
               ...
               "содержимое). `before=`: повторная сборка "
    ...
    "решение": Seam(
        name="решение",
        neighbor="orchestrator",
        address="orchestrator.live.solve, tools.py. Сведено",
        ...
        awaits="петля с инструментами живого режима есть (правка блоков, чтение "
               "материалов, схемы), и в наборе есть `put_source`: "
    ...
    "текст": Seam(
        ...
        awaits="проход текста одним вызовом по готовому списку блоков; "
               "блоки с source manual/file он выбрасывает сам",
    ...
    "проверка кода": Seam(
        ...
        address="код проверяется статически: orchestrator.doors.check_code. "
                "Сведено",
        ...
        awaits="дверь есть и зовётся стадией «сборка»: tree-sitter разбирает исходник, "
               ...
               "службе — сценарию оно не принадлежит",
    ...
    "артефакты": Seam(
        name="артефакты",
        neighbor="orchestrator",
        address="Project.put_artifact / resolve_artifact. Сведено",
    ...
    "состояние стадий": Seam(
        ...
        neighbor="orchestrator",
        address="Project.put_state / state. Сведено",
...
class Services:
    ...
    project: object = None
    ask: object = None                # orchestrator.ask
    make_template: object = None      # orchestrator.make_template
    solve: object = None              # уровень 3
