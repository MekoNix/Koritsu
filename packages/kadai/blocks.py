...
def to_pdf(data: bytes) -> bytes:
    ...
    беда прилетает исключением, а PDF в архив просто не кладётся (записка Ж:
    «отчёт.pdf — оттуда же, если просили и LibreOffice есть»). Врать о наличии
    PDF нельзя, а отказываться собирать архив из-за него — тем более.
    """
...
def problem_dict(p) -> dict:
    ...
    Четвёртого канала предупреждений не заводится (записка З.12), но приходят
    они и объектами (`kyotsu.Notice`), и словарями — от того, кто их отдал.
