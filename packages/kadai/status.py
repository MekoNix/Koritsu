...
HTTP здесь не проектируется и не пишется. Отдаётся снимок плюс курсор — форма
фиксированная, ключ в ключ. Ключи латиницей, значения по-русски: ключи читает
сайт, значения — человек.
...
def snapshot(work: Work, *, spent: dict | None = None, problems=(), since: int = 0) -> dict:
    ...
    return {
        ...
        # Условие так, как его прочитали: скан распознаётся, и распознанное
        # показывается человеку **до** работы — ошибка OCR в одной формуле
        # даёт безупречно решённую чужую задачу, и заметить её может только
        # он. Поэтому текст лежит в снимке, а не «его можно запросить
        # отдельным вызовом»: то, за чем надо идти вторым запросом, не
        # показывают.
