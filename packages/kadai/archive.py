...
ещё один документ работы — его открывали и искали в нём то, что можно сдать.
Запись о сборке документом не является: это метаданные архива, и точка в начале
имени говорит об этом сразу, до открытия. Содержимое от переименования не
изменилось ни на строку — обязательная строка про незапущенный код на месте.
...
SOLUTION = "решение.md"
# Запись о сборке — скрытым файлом: она метаданные архива, а не документ
# работы, и лежать в одном ряду с отчётом и решением ей нельзя (см. заголовок).
NOTICE = ".metadata"
...
# Расширения, по которым файл в папке решения читается исходником. Тот же
# список, что и в `SOURCE_EXT`, а не второй: разойдясь, они дали бы архив, в
# котором `.c` человека лежит, а `.cs` — нет. Заголовки C и C++ добавлены
# отдельно: своего языка у них нет, а без них в архиве оказывается половина
# программы.
SOURCE_EXTS = frozenset({f".{ext}" for ext in SOURCE_EXT.values()} | {".h", ".hpp"})
...
def is_source_name(name) -> bool:
    """Похоже ли имя файла на исходник. Смотрим на расширение, и только на него.

    Расширение — то же, по чему исходник узнаёт человек в распакованной папке и
    по чему `check_code` выбирает разбор. Заглядывать в содержимое, чтобы
    решить, «код ли это», значило бы завести в сценарии второе мнение о языках —
    знание, которое ему не принадлежит.
    """
    name = unicodedata.normalize("NFC", str(name or "")).strip().lower()
    точка = name.rfind(".")
    return точка > 0 and name[точка:] in SOURCE_EXTS


def unique_name(name: str, taken) -> str:
    """Имя, которого в архиве ещё нет: столкнувшемуся приписывается номер.

    Столкновение имён — ошибка описи (`check_names`), и для описи это верно:
    молча пропавший файл человек находит на проверке. Но исходники приходят из
    двух мест сразу — из блоков работы и из файлов папки решения, — и два
    разных файла с одним именем там законны. Уронить из-за этого весь архив
    было бы хуже, чем назвать второй файл «sort-2.py»: номер видно, пропажу нет.

    `taken` — множество уже занятых имён без учёта регистра и нормализации; оно
    пополняется здесь же, потому что сравнивать имена иначе, чем `check_names`,
    нельзя: разойдясь, мы вернули бы то самое столкновение.
    """
    leaf = safe_leaf(name)
    точка = leaf.rfind(".")
    ствол, ext = (leaf[:точка], leaf[точка:]) if точка > 0 else (leaf, "")
    n, кандидат = 1, leaf
    while unicodedata.normalize("NFC", кандидат).casefold() in taken:
        n += 1
        кандидат = f"{ствол}-{n}{ext}"
    taken.add(unicodedata.normalize("NFC", кандидат).casefold())
    return кандидат
...
def plan_archive(*, notice: str, report: str | None = None,
    ...
    пары `(имя файла, текст)`: код, напечатанный в работе, живёт блоком, а не
    материалом, и нести его в архив приходится текстом; текстом же приходят
    файлы исходников из папки решения — и те, что положила модель, и те, что
    принёс человек.

    `diagrams` — пары `(ключ блока, идентификатор)`: схема называется тем
    блоком, который на неё ссылается, иначе по архиву не понять, какая схема к
    какому разделу.
