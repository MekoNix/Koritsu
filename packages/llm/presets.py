...
def deepseek(endpoint_id: str = "ep_deepseek", model: str = "deepseek-chat",
    ...
    Отдельно про имя модели. Живая проба 2026-09-03 предупредила, что
    `deepseek-chat` **нет** среди трёх моделей, которые отдаёт `/v1/models`, —
    и вызовы при этом прошли. Имя здесь остаётся как есть (оно работает), но
    это ровно тот случай, ради которого шаг 1 пробы кладёт предупреждение в
    `Probe.warnings`: переименование у поставщика мы должны узнать здесь, а не
    отказом на боевом вызове.
    ...
    **Главная особенность, ради которой стоит читать эту докстроку.** Через
    ...
    spec = EndpointSpec(
        ...
        declared=Declared(
            ...
            # Шлюз выбирает исполнителя на каждом запросе — это и есть вся его
            # суть, и та самая тройка (endpoint, модель, провайдер) из докстроки
            # выше. Пометка не запрещает ничего: она проставляет в результате
            # пробы `provider_routed`, чтобы измеренное читалось как «так было
            # на том прогоне», а не как свойство endpoint'а навсегда. Без неё
            # ступень лестницы, снятая пробой на умеющем провайдере, выглядела
            # бы знанием — а следующий запрос уедет к другому исполнителю.
