...
этого каталога — ошибка, которую придётся выкорчёвывать.
...
class Request:
    ...
    выбирает, а исполняет: выбор — общее решение, оно в structured.py. Картинок
    здесь нет намеренно: картинки модели не отдаются вовсе, OCR справляется.
    Если endpoint объявит зрение, слой всё равно им не пользуется.
    """
