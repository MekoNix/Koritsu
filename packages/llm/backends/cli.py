...
ключа к API нет, а пробовать надо уже сейчас, и подписка Claude Code —
единственный доступный путь к модели. Всё, что ниже, написано с оглядкой на то,
что путь этот кривой и его придётся выбросить, как только появится ключ.
...
`--disallowed-tools '*'` здесь НЕ используется намеренно, хотя первая проба шла с
ним. Этот параметр — список правил запрета для системы разрешений («Bash(git *)»,
«Edit»), а не выключатель набора инструментов, и то, что `*` совпадёт с именем
каждого, нигде не обещано. Незаявленная защита, которая выглядит защитой, хуже
отсутствующей: с ней перестают искать настоящую. Набор выключается `--tools ""`,
а `--restricted` вдобавок убирает всё, что запускает команды и код, запирает
файловые инструменты в рабочих каталогах и отказывается от `bypassPermissions`.
...
разобранный объект в поле `structured_output`). Но:
  ...
  * без схемы модель заворачивает ответ в забор ```json (проверено на просьбе
    «верни ровно этот JSON и ничего больше»).
...
class CliBackend(Backend):
    ...
    def build_body(self, request: Request, stream: bool = True) -> dict:
        ...
        просьбе: молчаливая деградация — худший класс отказа.
        """
