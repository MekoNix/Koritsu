...
Несущая мысль всего пакета: **возможность — свойство пары (endpoint, модель), а
не свойство протокола.** Поэтому `protocol` здесь — только «как разговаривать по
проводу», а что endpoint умеет, лежит отдельно в `declared` (заявка владельца) и
`probe` (что подтвердил пробный вызов). Код никогда не спрашивает «это
DeepSeek?» — он спрашивает capabilities().
...
class Structured:
    ...
    Порядок именно такой, а не «сначала режим JSON»: строгий инструмент даёт
    почти те же гарантии, что схема (модель не может выдумать ключ), тогда
    как json_object гарантирует только валидный JSON — ключи и типы всё равно
    проверяет наш валидатор. Поэтому tool_strict выше.
    """
...
class Probe:
    ...
    # Записи — `kyotsu.Notice` (module `llm`), та же форма, что у замечаний
    # остальных пакетов: до 2.0.0a4.2 здесь лежали голые строки, у которых нет
    # ни уровня, ни кода, и отобрать по ним в интерфейсе было нечего.
