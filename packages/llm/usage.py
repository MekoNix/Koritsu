...
Лимит считаем в токенах. Сложить четыре числа из usage в одно нельзя: токен
входа, токен из кэша и токен выхода отличаются в цене до 50 раз. Поэтому лимит
считается в **приведённых единицах** — «сколько это стоило бы, будь всё
входными токенами этого endpoint'а»:
