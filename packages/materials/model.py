...
клиента, и модель, поэтому там `page`, а не «страница» (решение владельца:
внутри всё по-английски). Но карточку и якорь читает человек, и якорь уезжает
в отчёт на русском, поэтому подписи лежат здесь же — `KIND_WORDS` и
`UNIT_WORDS`. Одно место на оба перевода: разойдись они, «страницы 3–7» в
отчёте и `page` в карточке начали бы спорить друг с другом на глазах у автора.
"""
