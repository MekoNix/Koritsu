...
def supported(name: str, data: bytes) -> bool:
    """Возьмётся ли разбор за этот файл — тот же вопрос, что решает `parse`,
    но без самого разбора.

    Нужно тому, кто принимает файл снаружи (служба `api`): отказать
    «такой тип не поддерживается» надо **до** запуска разборщика, а решение
    «поддерживается» обязано быть одним и тем же в обоих местах. Второй список
    расширений в службе разошёлся бы с этим на первом же добавленном сюда `.f90`,
    и человек получал бы отказ на файл, который пакет отлично читает.

    `data` нужен не для красоты: у файла с незнакомым расширением тип решается
