...
# Что ищет быстрый отбор абзацев: свой тег, ссылка `{ref:}` и комментарий Jinja
# `{# … #}` — последний потому, что комментарий вырезается из документа при
# сборке, а вырезать его может только тот, кто абзац увидел.
MARKERS = ("{{", "{ref:", "{#")
