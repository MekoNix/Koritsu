...
Несущее решение (`ref/koritsu-hokoku-live-2026-09-03.md` §В.1): **список — источник,
документ — сборка, а не наоборот.** Мутировать готовый DOCX нельзя, довод один и
решающий: пропадает отмена, а в совместном письме она нужна каждый ход. Отсюда всё
остальное здесь: `Work` неизменяем, каждая операция возвращает новый список, прежний
цел и годится для «верни как было» без единой копии на стороне вызывающего.
...
с версиями хранит `orchestrator` (§З.3), модель зовёт он же — `live_tools()` отдаёт
...
# Виды, в которые пишется связный текст одним проходом (решение владельца 2026-09-04).
...
class Block:
    ...
    сколько угодно и ничего не порвётся (§К.2).
    """
...
def new_key(work: Work, *, prefix: str = "b") -> str:
    ...
    Ключ непрозрачный, а метка отдельно (§К.2): читаемый ключ соблазняет переименовать,
...
def replace(work: Work, key: str, value=_KEEP, *, label=_KEEP) -> Work:
    ...
    Это «переделай этот блок» из §Д.2: соседи не пересчитываются — иначе точечная
...
def rename(work: Work, key: str, new_key: str) -> Work:
    ...
    текстах и подписях — беда, ради которой §К.2 предлагала ключ вовсе не менять.
...
def assemble(work: Work, *, style: dict | None = None, profile=None,
    ...
    страниц считает Word, и обещать совпадение нельзя (§Д.1).
    """
...
def text_slots(work: Work) -> list[dict]:
    ...
    Решение владельца 2026-09-04: скелет работы, таблицы, код и схемы набираются петлёй
    по блокам, а **весь связный текст пишется одним проходом** по готовому списку, видя
    соседей. Иначе абзацы, написанные по одному, разойдутся между собой, и это уже
    посчитано (`ref/koritsu-kadai-2026-08-31.md` §Г.3).
