...
Тип тега объявляет манифест, а не имя ключа (решение 2026-08-28: имя-префикс вроде
`global_` ломает `{ref:ключ}` и сохранённые значения при смене типа, а промпт девать
некуда). По умолчанию всё `markdown` — он покрывает текст, заголовки, списки, таблицы
и картинки; манифест объявляет исключения (`image`, `diagram`, `code`, `formula`,
`table`, `toc`).
...
def manifest_from_template(template, *, base: Manifest | None = None,
    ...
    Обновление устроено так (решение 2026-08-29): тег добавили — заводим запись с
    `markdown` и без промпта; тег убрали — ставим `missing: true`, но **не удаляем**:
    промпт и значения переживут случайную правку шаблона; изменилась только метка —
    обновляем метку, промпт не трогаем. Переименование мы не видим (оно выглядит как
    «убрали и добавили») и молча переносить промпт не имеем права: два тега, поменянных
    местами в Word, дали бы перепутанные значения, и никто бы этого не заметил.
    Подсказку об этом даёт `check_manifest`.
    """
    tags: dict = {}
