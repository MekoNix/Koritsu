...
def _refs(values: dict, tags: dict, *, what: str = "тега",
    ...
    `level` — насколько это беда, и он различается по делу (решение владельца
    2026-09-04). В живом режиме цель ссылки видна точно: номер получают только
    нумерованные блоки (`live.ref_targets`), и промах — `error`, из-за которого
    прогон до архива не доходит: «?» в готовом документе хуже отказа. В шаблонном
    режиме значения цели во время проверки может не быть вовсе, и `warning` там
    остаётся тем, чем был, — предупреждением о том, что проверить нечем.
    """
    known = set(tags)
