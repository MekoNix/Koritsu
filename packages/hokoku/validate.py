...
в собранный отчёт без единого слова, и обнаруживал это тот, кто отчёт принимал.
Единственные потолки
...
def validate(template, values: dict, manifest: Manifest) -> list[Problem]:
    ...
    доезжает до готового DOCX молча — и обнаруживается уже у того, кто его читает.
...
def _tag(key: str, spec, values: dict, filled: set, tags: dict) -> list[Problem]:
    ...
    if _empty_value(value):
        # «модель ничего не вернула» выглядело ровно как «тег заполнен»; render
        # считает это ошибкой, и знать о ней надо до сборки
...
def _refs(values: dict, tags: dict, *, what: str = "тега",
          level: str = "warning", labels: dict | None = None) -> list[Problem]:
    ...
    `labels` — заголовки блоков по ключу. Замечание читает человек, а `b-07` ему
    ничего не говорит: он ищет раздел глазами по названию. Ключ при этом остаётся
    в тексте — им адресуется правка, — а название приписывается рядом.

    Текст замечания называет три вещи: откуда ссылка, куда она вела и что будет в
    документе. Ни одна из них по остальным не восстанавливается: без адреса
    источника править нечего, без имени цели непонятно, что искать, а без «?» не
    видно, почему это вообще беда.

    `level` — насколько это беда, и он различается по делу. В живом режиме цель
    ссылки видна точно: номер получают только нумерованные блоки
    (`live.ref_targets`), и промах — `error`, из-за которого прогон до архива не
    доходит: «?» в готовом документе хуже отказа. В шаблонном режиме значения
    цели во время проверки может не быть вовсе, и `warning` там остаётся тем, чем
    был, — предупреждением о том, что проверить нечем.
    """
    known = set(tags)
    ...
    for key, v in values.items():
        адрес = _addressed(key, (labels or {}).get(key))
        for name in dict.fromkeys(REF_TEXT_RE.findall(_ref_text(v))):
            if name not in known:
                out.append(_p(level, "unresolved_ref", key,
                              f"ссылка {{ref:{name}}} из {what} {адрес} никуда не ведёт: "
                              f"{what} «{name}» в работе нет, и в документе на его "
                              f"месте будет «?»{_hint(name, known)}",
                              got=name))
    return out


def _addressed(key: str, label: str | None = None) -> str:
    """Как назвать адресата человеку: «Реализация» (b-07) или просто b-07."""
    заголовок = str(label or "").strip()
    return f"«{заголовок}» ({key})" if заголовок else f"{key}"
