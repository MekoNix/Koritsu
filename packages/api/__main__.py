...
Четыре команды, и все четыре — то, что иначе делалось бы руками в интерпретаторе:
    ...
    worker    крутить очередь заданий (второй контейнер в compose)
    migrate   довести базу до последней миграции и выйти
    purge     физически убрать из корзины всё, чей срок вышел
...
с restic. Её же зовёт воркер раз в час; команда остаётся для руки и
...
Пакеты в проекте не устанавливаются, поэтому запуск из корня
...
# Caddy, и умолчание, открывающее порт наружу, — это забытая настройка,
...
def purge(args) -> int:
    """Физически убрать из корзины всё, чей срок вышел (десять дней).
