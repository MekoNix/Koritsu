...
агенты B (аккаунты и сессии), C (workspace, проекты, корзина) и D (материалы и
...
**SQLite** (решение владельца §7): второй сервер, если появится, — исполнитель
...
`Row` даёт `id` строкой uuid4 (`ids.new_id`) — решение владельца о форме
...
**Как B/C/D добавляют миграции**

Один файл миграции на агента, не больше: два файла от одного агента — это две
ветки, которые он же и будет сливать. Порядок:
...
Агенты работают параллельно, поэтому у B, C и D `down_revision` окажется
одинаковым — это три головы, а не беда: их сливает агент E одной командой
...
def ensure_volume(settings: Settings) -> None:
    ...
    `migrations/env.py` (командная строка, то есть B/C/D с `--autogenerate`).
    """
