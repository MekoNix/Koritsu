...
def при_завершении(s: Session, задание) -> Notification | None:
    ...
    `None` в четырёх случаях, и все законные: задание ещё не кончилось,
    ...
    ронять закрытие задания), вид задания уведомлений не заводит
    (`jobs.registry.БЕЗ_УВЕДОМЛЕНИЯ`). Проверка вида — здесь, а не у трёх
    мест, которые зовут эту функцию: так её нельзя забыть в одном из них.
    ...
    """
    from ..jobs.registry import notifies                     # noqa: PLC0415

    статус = str(getattr(задание, "status", "") or "")
    вид = ПО_СТАТУСУ.get(статус)
    if вид is None or not notifies(str(getattr(задание, "kind", "") or "")):
        return None
