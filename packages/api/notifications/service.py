...
задание положило в `result`. Решение владельца §3 («экспорт — кнопка в проекте,
задание, скачивание из уведомления») требует, чтобы из строки колокольчика файл
скачивался, а не разыскивался: подробности — в докстроке `артефакты`.
...
def артефакты(результат) -> dict:
    ...
    уведомления». Скачивание из уведомления означает ссылку **в самом
