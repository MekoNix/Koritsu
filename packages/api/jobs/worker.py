...
а не поток внутри веб-сервера. Довод не про
...
2. захватить одно задание — **атомарно**, одним `UPDATE`, и тем же запросом
   списать цену его вида (`runs/limits`);
...
Отмена — закрытые теги остаются. Значит, сначала просьба: обработчик
...
(повтор упавшего — только вручную): обработчик, который упал, сказал
...
# Раз в час — уборка («уборка заданий старше 90 дней и корзины тем же
# воркером раз в час»).
УБОРКА_С = 3600.0
...
class Worker:
    ...
    def _взять(self, s: Session, job_id: str, user_id: str, kind: str) -> bool:
        ...
        **Здесь же списывается цена вида** (`runs/limits`): захват — это
    ...
    def _спец(self, job_id: str, kind: str) -> dict:
        ...
        Секрет кладётся сюда **только** видам, объявившим `needs_secret=True`
        (`registry`). Остальные получают настройки с секретом-заглушкой: `parse`
        ...
        Ключей поставщиков здесь нет вовсе: ключ у прогона свой, он лежит в базе
        зашифрованным и достаётся уже в подпроцессе (`runs/model.endpoint`).
        """
        поля = dataclasses.asdict(self.settings)
        if not registry.needs_secret(kind):
            поля["secret"] = СЕКРЕТ_ЗАГЛУШКА
        return {"job_id": job_id, "настройки": поля,
    ...
    def _закрыть(self, job_id: str, ответ: dict) -> None:
        ...
        with self.db.session_scope() as s:
            ...
            # Колокольчик (писем нет). В той же транзакции, что и
    ...
    def уборка(self) -> dict:
        ...
        Раз в час, и все три уборки вместе, а не в трёх местах: воркер —
        ...
        задание `parse`. Срок им — `job_timeout_s`, тот
