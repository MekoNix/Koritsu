...
**Чужое задание — 404, а не 403.** Тот же довод, что у чужого проекта (§3):
...
работа агента B ночи 2, и она встанет поверх той же выборки
(`service.события`). Здесь список нужен ровно затем, чтобы очередь была
проверяема и полезна до появления потока: клиент, у которого SSE не работает
...
def поставить(тело: JobIn, request: Request, s: SessionDep,
    ...
    лимит расхода против **цены этого вида** (§12) и отказывает
...
def карточка_одного(job_id: str, s: SessionDep, user: CurrentUser) -> dict:
    ...
    хранения (§7: «90 дней с последнего обращения»)."""
