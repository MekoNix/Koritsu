...
Решение владельца §7: «очередь — таблица в базе». Не Redis и не очередь в
памяти, и довод у владельца записан коротко — «переживает перезапуск». Довод
стоит развернуть, потому что он же объясняет половину полей ниже: очередь в
...
Правило то же, по которому значения тегов остались файлами (§8): база держит
...
`queued` (воркер умер, не доделав). Это не ретрай упавшего обработчика — §7
говорит «повтор упавшего только вручную», и это правило здесь соблюдено: подъём
...
Решение владельца §7: «90 дней с последнего обращения; потом удаляются и запись
...
class Job(Row):
    ...
    # Чьё задание. `CASCADE` — обещание §7: удалили аккаунт, значит и заданий
    ...
    # Просьба остановиться. Флаг, а не убийство процесса: §7 обещает «закрытые
    # теги остаются», а это возможно только если обработчик остановится сам —
    ...
    # Внутренние единицы, которыми человек платит за этот запуск (§7: «расход
    # наружу во внутренних единицах»; §12: «за наши инструменты, а не за токены
