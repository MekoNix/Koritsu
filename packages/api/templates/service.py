...
**Манифест лежит рядом с байтами, а не в базе.** Бланк, собранный из блоков
работы, несёт с собой описание своих тегов: тип, задание модели, потолки и
зависимости — всё, чего абзацем DOCX не сказать. Хранится оно файлом рядом
(`<id>.manifest.json`), потому что это запись движка отчётов целиком, со своей
схемой и своей версией; разложенная по колонкам, она разошлась бы с движком на
первой же его правке, и разошлась бы молча. Манифеста рядом может не быть —
так лежит всякий DOCX, принесённый человеком со стороны, и путь у такого бланка
прежний: теги читаются из самого файла.
...
# Расширение манифеста, лежащего рядом с байтами бланка. Файл рядом, а не
# колонка в базе: манифест — это запись движка отчётов целиком, со своей схемой
# и своей версией, и разложить её по колонкам значило бы завести вторую схему,
# которая разойдётся с первой на первой же правке. Рядом с DOCX он и потому, что
# описывает ровно эти байты: файла нет — и описывать нечего.
РАСШИРЕНИЕ_МАНИФЕСТА = ".manifest.json"
...
def путь_манифеста(settings: Settings, user_id: str, template_id: str) -> str:
    """Где лежит манифест этого бланка. Наружу путь не уезжает никогда."""
    return os.path.join(templates_dir(settings, user_id),
                        check_id(template_id, where="path.template_id")
                        + РАСШИРЕНИЕ_МАНИФЕСТА)


def манифест(settings: Settings, шаблон: ReportTemplate) -> dict | None:
    """Манифест, лежащий рядом с байтами бланка, или `None`.

    `None` — законное состояние и означает «манифеста рядом нет»: так лежит
    всякий DOCX, принесённый человеком со стороны. Его теги читаются из самого
    файла, как читались всегда.

    Битый файл — то же самое `None` со строкой в журнале: манифест рядом это
    подспорье, а не условие работы бланка, и отказать в выборе бланка из-за
    испорченного описания значило бы отнять у человека рабочий документ.
    """
    try:
        with open(путь_манифеста(settings, шаблон.user_id, шаблон.id),
                  encoding="utf-8") as f:
            запись = json.load(f)
    except FileNotFoundError:
        return None
    except (OSError, ValueError):
        беды.exception("шаблон %s: манифест рядом не читается", шаблон.id)
        return None
    return запись if isinstance(запись, dict) else None


def записать_манифест(settings: Settings, user_id: str, template_id: str,
                      запись: dict) -> None:
    """Положить манифест рядом с байтами бланка. Беда тома не роняет загрузку.

    Не роняет потому, что бланк без манифеста рядом остаётся рабочим бланком:
    теги в нём есть, и путь у него обычный. Отказ здесь стоил бы человеку уже
    собранного файла ради описания, которое можно собрать заново.
    """
    цель = путь_манифеста(settings, user_id, template_id)
    try:
        os.makedirs(os.path.dirname(цель), exist_ok=True)
        with open(цель, "w", encoding="utf-8") as f:
            json.dump(запись, f, ensure_ascii=False)
    except (OSError, TypeError, ValueError):
        беды.exception("шаблон %s: манифест рядом не записался", template_id)
...
def добавить(s: Session, settings: Settings, user_id: str, *, имя: str,
             имя_файла: str, данные: bytes,
             манифест_рядом: dict | None = None) -> tuple[ReportTemplate, bool]:
    ...
    `манифест_рядом` — описание тегов, приехавшее вместе с байтами (бланк,
    собранный из блоков работы). Ложится файлом рядом с DOCX и читается при
    выборе бланка, чтобы `type`, `prompt`, `limits` и `depends_on` не
    восстанавливались догадкой по метке. Тот же файл, загруженный второй раз, —
    тот же шаблон; описание к нему дописывается, только если его ещё не было:
    байты одни и те же, а переписывать чужую запись поверх незачем.
    ...
    if прежний is not None:
        if манифест_рядом is not None and манифест(settings, прежний) is None:
            записать_манифест(settings, user_id, прежний.id, манифест_рядом)
    ...
    with open(цель, "wb") as f:
        f.write(данные)
    if манифест_рядом is not None:
        записать_манифест(settings, user_id, шаблон.id, манифест_рядом)
    return шаблон, True
...
def удалить(s: Session, settings: Settings, user_id: str,
    ...
    for файл in (путь(settings, user_id, шаблон.id),
                 путь_манифеста(settings, user_id, шаблон.id)):
        try:
            os.remove(файл)
        except OSError:
...
def выбрать(s: Session, settings: Settings, project_id: str, каталог: str,
    ...
    Манифест, лежащий рядом с байтами, уезжает туда же. Он есть у бланков,
    собранных из блоков работы, и восстанавливает то, чего в DOCX не выразить:
    тип тега, задание модели, потолки и зависимости. Без него те же теги
    поднялись бы с типом, угаданным по метке, — то есть схема встала бы туда,
    где ждали таблицу. Бланк, принесённый человеком со стороны, манифеста рядом
    не имеет, и путь у него остаётся прежним.
...
__all__ = ["путь", "байты", "добавить", "мои", "найти", "по_ид", "удалить",
           ...
           "приложенный", "выбрать", "манифест", "путь_манифеста",
           "записать_манифест", "BAD_TEMPLATE", "INVALID_NAME", "NOT_FOUND",
           "РАСШИРЕНИЕ", "РАСШИРЕНИЕ_МАНИФЕСТА"]
