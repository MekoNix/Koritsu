...
Решение владельца §3: два потока. Технический — «id запроса, UUID пользователя,
маршрут, код, длительность»; события безопасности — отдельно (их заведёт B:
отказ входа, превышение лимита). Здесь первый, и правило у него жёсткое:
...
Что читать B/C/D: `request.state.request_id` — метка текущего запроса (кладите
её в события безопасности), `request.state.user_id` — UUID вошедшего, если
обработчик его выставил. Каркас его не выставляет: аккаунты — дело B. Пока не
выставлен, в журнале стоит `user=-`.
"""
