...
несуществующем владельце». Таблицу `users` заводит агент B; на SQLite ссылка на
ещё не созданную таблицу при `CREATE TABLE` проходит и начинает проверяться
только на первой вставке, так что порядок применения двух миграций (нашей и B)
роли не играет.
...
class WorkspaceMember(Base):
    ...
    # ещё поедет (§7 «решится на этапе UI»), а `CHECK` на SQLite меняется только
    # пересборкой таблицы.
...
__all__ = ["Workspace", "WorkspaceMember", "NAME_MAX"]
