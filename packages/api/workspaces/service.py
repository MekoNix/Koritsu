...
зовут и маршруты, и агент D (материалы), и уборщик корзины, который живёт вне
...
(агент B): `Callable[[Session, User], None]`. Ставится хук в `routes.collect()`,
...
# Список закрыт (§7): только `viewer` и `editor` плюс `owner` как создатель.
...
# (§7), а перевод «Личное» сделает интерфейс — он же знает флаг `personal`.
...
def require_role(s: Session, user_id: str, workspace_id: str,
    ...
    проектов и материалы агента D. Отказы — те самые два, что описаны в
...
def create_personal(s: Session, user) -> Workspace:
    ...
    хука B: `Callable[[Session, User], None]`. Идемпотентность здесь не
...
def register_hooks() -> bool:
    ...
    Возвращает, стоит ли хук на месте: `False` означает «аккаунтов ещё нет» — не
    беда, а состояние ночи, когда агент B работает параллельно.
    """
    try:
