...
том, вынужден чинить глобальное состояние. Тесты этой ночи заводят приложение по
штуке на тест — на это фабрика и рассчитана.
...
`migrate`, иначе `Base.metadata` пуст и `--autogenerate` у B/C/D выпишет
`DROP TABLE` вместо новой таблицы.
...
Чего здесь нет намеренно: очереди и воркера, SSE, внешних токенов, CORS. CORS не
нужен по решению владельца §3 (сайт и API на одном домене за Caddy); исключение
— админка, у неё отдельный origin, и её здесь тоже нет.
"""
...
def create_app(settings: Settings) -> FastAPI:
    ...
    # Живёт в `routes`, потому что про входы знает он (§5: наружу
    ...
    def health() -> JSONResponse:
        """Жив ли процесс и отвечает ли база (решение владельца §7).
