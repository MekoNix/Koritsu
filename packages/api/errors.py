...
Решение владельца §5: наружу уходит `{code, message, where}` — тот же дух, что у
...
* **наружу — английский** (решение владельца §7: «коды, тексты ошибок,
...
красоты документа: из неё генерируется клиент сайта (§5), и без неё генератор
...
# становятся именами типов в клиенте сайта (§5).
...
# Коды, которые выдаёт сам каркас. B/C/D заводят свои и перечисляют их у себя.
NOT_FOUND = "not_found"
