...
Решение владельца §3: журналов два — технический (id запроса, UUID
пользователя, маршрут, код, длительность) и **события безопасности** (отказ
входа, замок, превышение лимита, отказ CSRF, отозванный ключ). Ночью 1 второй
поток писался только в `logging`-логгер `api.security`; §11 просит показывать
его в админке, а показать поток, ушедший в stderr контейнера, нельзя — значит,
у него появляется таблица.
...
Это дословно §7: при удалении аккаунта журнал с его UUID уходит, «для журнала
безопасности оставить только обезличенную запись». Каскад стёр бы саму запись —
...
пароля, ни строки токена, ни значения cookie, ни почты (§7: почта — то, что при
...
__all__ = ["SecurityEvent", "IP_LEN", "KIND_LEN", "LOGIN_FAILED",
           "ACCOUNT_LOCKED", "REGISTRATION_RATE_LIMITED", "RATE_LIMITED",
           "CSRF_FAILED", "TOKEN_REVOKED"]
