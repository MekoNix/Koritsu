...
class User(Row):
    ...
    # openrouter), а не ссылка на строку ключа: общий ключ службы строки в базе
    # не имеет вовсе, и ссылка не выразила бы «плачу общим». `NULL` — «ничего
    # не выбрано», и это не то же самое, что выбранный и потом отозванный: в
    # первом случае сайт подставляет своё правило «сначала свой ключ, потом
    # общий», во втором показывает выбор человека как есть.
