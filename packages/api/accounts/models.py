...
Четыре таблицы, и три из них существуют потому, что иначе не выполняются
обещания службы:
...
NICK_LEN = 32            # предел ника (границы — `service.годный_ник`)
...
ENDPOINT_LEN = 64        # имя пресета модели (`llm.presets`), как у `model_keys`
...
class User(Row):
    ...
    `plan` — строка, а не таблица подписок: подписку включает владелец
    службы в админке, а платёжка встанет вебхуком позже. Поле заводится
    сразу, чтобы этот вебхук не потребовал правки модели.

    `totp_secret` и `totp_enabled` — «заложить, интерфейса нет». Секрет
    ...
    `deleted_at` — корзина аккаунта (10 дней, потом полное удаление). Сама
    уборка не здесь: её место рядом с уборкой корзины проектов.

    `blocked_at` — блокировка владельцем службы. Третье состояние рядом с
    двумя прежними, и все три разные: запертый перебором (`locked_until`)
    отпускается временем, удалённый (`deleted_at`) убрал себя сам,
    заблокированного пускает обратно только владелец.

    `nickname` и `nickname_key` — имя человека на экране: имени и фамилии
    нигде нет, у аккаунта есть ник. **Две колонки, а не
    одна,** и это не запас: ник обязан быть уникальным без учёта регистра, а
    единственный способ потребовать это от базы — уникальный ключ по
    приведённому значению. Выражение `lower(nickname)` для этого не годится:
    `lower()` в SQLite приводит только ASCII, то есть `Ник` и `ник` остались бы
    для базы разными строками, а кириллица владельцем разрешена. Поэтому
    приведение делает Python (`str.casefold`), а в базе лежит его результат —
    `nickname_key`, по нему и стоит уникальность. `nickname` при этом хранится
    так, как человек его набрал: показывать `КириСу` как `курису` — значит
    ...
    email: Mapped[str] = mapped_column(String(EMAIL_LEN), unique=True,
                                       index=True)
    nickname: Mapped[str] = mapped_column(String(NICK_LEN))
    nickname_key: Mapped[str] = mapped_column(String(NICK_LEN), unique=True,
                                              index=True)
    ...
    # Админ службы («флаг `is_admin`»). Поле, а не таблица ролей: роль
    ...
    # сегодня: `monthly_units` (расход модели), `quota_bytes` (место
    # на томе). Пустой мешок означает «как у всех», а не «ноль».
    limits: Mapped[dict] = mapped_column(JSON, default=dict)
    # Умолчания панели агента (раздел настроек «Агент и
    # модели»). Две колонки, а не мешок JSON рядом с `limits`: это не лимиты
    # владельца, а выбор человека, и читает их сайт на каждом открытии панели —
    # поле в ответе `me` честнее, чем ключ в чужом мешке.
    #
    # `default_endpoint` — **имя пресета** (`llm.presets`: deepseek, anthropic,
    # openrouter), а не ссылка на строку ключа: ключ человек меняет, не меняя
    # выбранного поставщика, и ссылка ломалась бы на каждой такой смене. `NULL`
    # — «ничего не выбрано», и это не то же самое, что выбранный и потом
    # отозванный: в первом случае сайт подставляет своё правило «ключ,
    # заведённый первым», во втором показывает выбор человека как есть.
    default_endpoint: Mapped[str | None] = mapped_column(
        String(ENDPOINT_LEN), default=None)
    # Переписывать ли агенту ручные правки человека. Умолчание — «нет»: молча
    # затёртая правка замечается на готовой работе, и цена ошибки здесь
    # несимметрична.
    agent_overwrite: Mapped[bool] = mapped_column(Boolean, default=False)
    # Своя картинка вместо генеративного аватара. В базе — **счётчик замен**, а
    # не имя файла и не флаг: файл на томе всегда один и называется всегда
    # одинаково (`accounts/avatar.py`), а числу здесь есть работа, которой флаг
    # не сделает. Ноль значит «своей картинки нет», и тогда аватар рисует сайт
    # из идентификатора. Всякая замена увеличивает счётчик на единицу, и он
    # уезжает в адрес картинки (`?v=`): без него браузер держал бы прежний
    # аватар до конца срока кэша, то есть человек менял бы картинку и не видел
    # изменения. Отдельная колонка «есть ли аватар» рядом с этой развелась бы с
    # ней на первой же гонке.
    ...
    deleted_at: Mapped[datetime.datetime | None] = mapped_column(
        DateTime(timezone=True), default=None)
    # Блокировка аккаунта владельцем службы. Отметка
    # времени, а не флаг: «когда заблокировали» — первый вопрос при разборе
    # жалобы, и хранить его отдельной колонкой рядом с флагом значило бы
    # завести два поля, одно из которых однажды забудут поставить.
    #
    # **Это не `locked_until` и не `deleted_at`.** `locked_until` ставит сама
    # служба на четверть часа за перебор пароля и снимает временем;
    # `deleted_at` — корзина, куда человек кладёт себя сам. Блокировку ставит
    # и снимает только владелец, и срока у неё нет.
...
class RegistrationAttempt(Row):
    """Сколько регистраций пришло с этого адреса за эти сутки.
...
__all__ = ["User", "EmailToken", "UserSession", "RegistrationAttempt",
           ...
           "EMAIL_LEN", "NICK_LEN", "HASH_LEN", "TOKEN_HASH_LEN", "IP_LEN",
           "UA_LEN", "ENDPOINT_LEN"]
