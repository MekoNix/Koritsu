...
**Писем служба не шлёт, Resend не подключён.** Подтверждение почты работает
консольным бэкендом, который печатает ссылку в журнал. Отсюда весь модуль:
протокол `Mailer` и `ConsoleMailer`.

Протокол, а не одна функция, — потому что мест, где письмо отправляется, будет
несколько (подтверждение, сброс пароля, «отчёт готов»), а способов отправки —
два (Resend и SMTP как запасной). Без протокола второй
...
**Что попадает в журнал.** Каркас (`log.py`) запрещает писать в технический
...
def mailer_for(settings: Settings) -> Mailer:
    ...
    Пока всегда консольный. Здесь же появится Resend — одной
...
# ── тексты писем ─────────────────────────────────────────────────────────────
#
# По-английски («всё на английском: коды, тексты ошибок, документация»).
