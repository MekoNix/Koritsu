...
URL берётся из `Settings`, а не из `alembic.ini` (решение владельца К.2: путь
тома задаётся снаружи). Два источника URL — это ровно та беда, ради которой
...
* `Settings.from_env()` — когда Alembic зовут из командной строки
  (`alembic revision --autogenerate`), например B/C/D, когда заводят таблицы.
...
# Пакеты в этом проекте не устанавливаются (решение владельца): импорт идёт
