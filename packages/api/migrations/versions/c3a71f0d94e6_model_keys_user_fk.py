...
Миграция сведения (агент E). Столбец `model_keys.user_id` завёл агент D, а
таблицу `users` — агент B, и оба в одну ночь: объявленный тогда внешний ключ
означал бы, что миграция D не применяется без миграции B, а тесты D не проходят
без таблицы B. Столбец поэтому был заведён готовым к ключу (тот же тип и длина),
а сам ключ отложен до слияния ветвей — то есть до этого файла.

Ставится он не для порядка. §7 обещает: удалили аккаунт — удалено всё, что с ним
