"""model keys — таблица ключей моделей.
...
на диске он лежит зашифрованным секретом сервера (`api/keys/crypto.py`).
...
миграция аккаунтов, и объявленный отсюда `ForeignKey` означал бы, что две
параллельные миграции требуют друг друга. Ключ с `ON DELETE CASCADE` ставится
отдельной миграцией при слиянии ветвей — и поставить его нужно: при удалении
аккаунта удаляется всё, что с ним связано, а ключ модели — ровно то, что не
должно пережить своего хозяина.
"""
