...
**Имя файла — с тома, а не из запроса.** Человек узнаёт свой документ в
«Загрузках» по имени, и шестнадцать hex там ничего не говорят. Поэтому имя
записывается при появлении артефакта (`Project.put_artifact(filename=)`, рядом
с артефактом) и отдаётся отсюда:

* загруженный бланк — под именем, с которым его принесли (файла или шаблона с
  полки);
* собранный отчёт, DOCX и PDF, — под именем отчёта, а у безымянного — работы;
  у обоих видов имя одно, расширение своё;
* материал — под своим именем из хранилища материалов;
* у кого имени нет (артефакты, положенные до появления имён, схемы, архивы) —
  под именем работы; идентификатор остаётся только на случай, когда не
  осталось и его.

Имя из запроса по-прежнему не принимается: клиент называл бы чужим байтам своё
имя. Записанное — не путь: от него остаётся последняя часть без управляющих
знаков и знаков направления письма, расширение снимается и ставится по
содержимому (DOCX, записанный «Отчёт.pdf», не уедет PDF'ом), а в заголовок оно
попадает через `заголовок_имени` двумя формами RFC 6266.

**Тип определяется по содержимому, а не по имени.** Имя пишет человек или
сборка, и оно не обязано совпадать с байтами, — а тип решает, как с файлом
обойдётся браузер, поэтому верить можно только первым байтам. Тип при этом не значит «браузер,
...
и то, и другое. Имя в заголовке остаётся и при `inline`: встроенный
просмотрщик подставляет его, когда человек нажимает «сохранить».
...
# Расширения, которые снимаются с записанного имени перед тем, как поставить
# своё по содержимому. Список, а не «всё после последней точки»: у «ЛР 1.2» и
# «Отчёт v2.1» точка — часть имени, и срезать её значило бы назвать файл «ЛР 1».
СНИМАЕМЫЕ = frozenset({".pdf", ".docx", ".doc", ".odt", ".rtf", ".xml", ".drawio",
                       ".png", ".jpg", ".jpeg", ".zip", ".txt", ".bin"})

ИМЯ_МАКС = 120

# Управляющие знаки, разделители путей обеих систем и знаки направления письма:
# `фото<U+202E>fdp.exe` в «Загрузках» выглядит PDF'ом.
_НЕ_ИМЯ = re.compile("[\\x00-\\x1f\\x7f-\\x9f/\\\\\\u200b-\\u200f\\u2028-\\u202e\\u2066-\\u2069]+")
...
def основа_имени(сырое: str | None) -> str:
    """Имя → основа без пути, мусора и известного расширения. Пусто — имени нет."""
    имя = _НЕ_ИМЯ.sub(" ", сырое or "")
    имя = re.sub(r"\s+", " ", имя).strip().lstrip(". ").strip()
    основа, расширение = os.path.splitext(имя)
    if расширение.lower() in СНИМАЕМЫЕ:
        имя = основа.strip()
    return имя[:ИМЯ_МАКС].strip()


def имя_файла(проект: Проект, artifact_id: str, расширение: str) -> str:
    """Под каким именем отдать артефакт: своё, иначе работы, иначе идентификатор.

    Порядок и доводы — в шапке модуля. Расширение всегда по содержимому.
    """
    основа = (основа_имени(открыть(проект).artifact_name(artifact_id))
              or основа_имени(проект.name) or artifact_id)
    return f"{основа}{расширение}"
...
def скачать_артефакт(artifact_id: str, проект: ЧитательПроекта,
    ...
    Имя берётся с тома (`имя_файла`), а не от клиента: записанное при
    появлении артефакта, иначе имя работы. Путём оно не становится, и
    человек по нему узнаёт свой документ в папке «Загрузки».

    `inline` — просьба показать, а не скачать; исполняется только для
    `ПОКАЗУЕМЫЕ` типов (см. шапку модуля), для прочих молча остаётся
    вложением. Имя стоит в обоих случаях.
    """
    ...
    return Response(content=данные, media_type=тип,
                    headers={
                        "Content-Disposition":
                            заголовок_имени(имя_файла(проект, artifact_id, расширение),
