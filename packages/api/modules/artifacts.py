...
покажи это»: ответ всегда `attachment` и всегда с `X-Content-Type-Options:
...
@router.get("/{artifact_id}", operation_id="download_artifact",
            ...
            description=(
                ...
                "follows the bytes; the answer is always an attachment and is "
                "never sniffed by the browser. Viewer role. 400 invalid_id, "
                "404 not_found."),
            response_class=Response)
def скачать_артефакт(artifact_id: str, проект: ЧитательПроекта) -> Response:
