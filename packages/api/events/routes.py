...
**Почему потоков два, а не один.** `GET /api/jobs/{id}/events` — прогресс и
текст одного задания; `GET /api/events` — один поток на пользователя для
уведомлений и колокольчика. Разница не в
