"""
routes — два входа службы и место, куда B/C/D вешают свои маршруты.

Решение владельца §1: не две службы, а **один код обработчиков и два входа**.
`/api/` — для сайта (cookie-сессия, CSRF), `/api/v1/` — для внешних скриптов и
агентов (Bearer-токен со scope'ами и своим лимитом). Довод владельца записан там
же и стоит того, чтобы повторить его здесь, где соблазн разъехаться живёт: две
отдельные службы означали бы два набора правил про проекты и файлы, «и они
разошлись бы молча». Наружу документируется и версионируется только `/api/v1/`.
...
**Как B/C/D добавляют маршруты.** Свой роутер в своём модуле, потом одна строка
в `collect()`:

    # packages/api/accounts/routes.py            (агент B)
...
**Как маршрут выставляется под `/api/v1/`** (ночь 2). Одна строка — имя роутера
...
и админка (§11: «Аккаунты и ключи моделей через токен не даём»). Отказ там —
`401 token_not_allowed`, и стоит он не здесь, а в `auth.deny_bearer`: список
закрытых входов — решение владельца, и читаться оно должно целиком в одном
месте, а не собираться из аргументов четырёх включений.
...
# приложений, а OpenAPI ругался на дубли operation id (нашёл агент C, ночь 1).
...
def collect() -> list[APIRouter]:
    """Все роутеры службы по порядку. Сюда B/C/D дописывают свои — по строке.
    ...
    # ── сюда B/C/D дописывают свои роутеры ──────────────────────────────────
    from .accounts.routes import router as accounts        # B: вход и сессии
    from .projects.routes import router as projects        # C: проекты, корзина
    from .workspaces.routes import router as workspaces    # C: workspace, роли
    from .workspaces.service import register_hooks         # C: личное при регистрации
    ...
    from .keys.routes import router as keys                 # D: ключи моделей
    from .materials.routes import router as project_files   # D: материалы проекта

    site.include_router(keys)
    site.include_router(project_files)

    from .jobs.routes import router as jobs                 # A5.1: очередь заданий

    site.include_router(jobs)

    # B5.1: прогоны (расход и лимит), потоки событий, колокольчик, версии тегов.
    ...
    from . import billing                                   # C5.1: заглушка платёжки
    from .admin.routes import router as admin               # C5.1: админка
    from .tokens.routes import router as tokens             # C5.1: внешние ключи

    site.include_router(tokens)
    site.include_router(admin)
    site.include_router(billing.router)

    from .export.routes import router as export             # D: выгрузка файлов

    site.include_router(export)

    # F: модули (реестр, блок-схемы, UML) и общее скачивание артефактов. Одной
    ...
    # (§11). Потоки событий (агент B) здесь есть: §11 обещает внешнему клиенту
    # не только задания, но и события. Права им не объявлены и не нужны — оба
    # `GET`, то есть `projects:read` по умолчанию: читать события — чтение, а
    # не запуск.
    #
    # Выгрузка (`export`) выставлена наружу при сведении ночи 2. Довод — тот
