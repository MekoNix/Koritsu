...
**Загрузка отвечает `202`, а не `201`** (§11, 2.0.0a5.1): разбор чужого файла
...
номер. Синхронного пути не осталось ни для каких файлов — решение владельца
§11 дословно: «один путь, через очередь (проще, и один код)».
...
async def загрузить(request: Request, s: SessionDep, user: CurrentUser,
    ...
    §4). Различать эти два случая кодом ответа значило бы рассказывать, какие
    файлы уже кто-то загружал.

    Настройки уезжают в постановку ради месячного лимита (§12): разбор — такой
...
def удалить(material_id: str, проект: РедакторПроекта) -> dict:
    ...
    Корзины у материала нет намеренно (§2 заводит её проекту и аккаунту):
