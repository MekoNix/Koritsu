"""
deps — единственная дверь к соседям: кто пришёл (зона B) и чей это проект (C).

Смысл файла в слове «единственная». Аккаунты заводит агент B, проекты и роли —
агент C, а материалы обязаны работать с обоими и при этом собираться, пока
кто-то из них ещё пишет свой пакет. Дверь одна, поэтому:
...
**Разрез между «спросить у C» и «решить самим» проходит по строке `проект_и_роль`.**
Ниже неё — знание C: где каталог проекта, в каком пространстве он лежит, кто его
завёл. Выше — политика отказов, и она наша, потому что её проверяют наши тесты:
...
* **404 `not_found`** — нет такого проекта или спрашивающий не участник. Один и
  ...
  спрашивающему не позволено (§3, «клиент видит только идентификаторы»).
...
FORBIDDEN = "forbidden"
# Зона C ещё не собрана. Отдельный код, а не пятисотка: это не поломка службы, а
# состояние ночи, и различать их обязан тот, кто читает журнал утром.
NOT_READY = "not_ready"
...
class Проект:
    ...
    `dir` — каталог проекта на томе. Наружу он не уезжает никогда: это ровно то,
    что запрещает §3, и живёт он только внутри обработчика.

    `owner_id` — **владелец проекта, а не пришедший**: квота считается на того,
    кто проект завёл (§2, «чужие проекты в общем workspace едят место того, кто
    их создал»).
    """
...
def _проекты():
    """Пакет проектов (агент C) или отказ `503`, пока его нет."""
    try:
        from .. import projects                              # зона C
...
def проект_и_роль(s: Session, settings: Settings, user_id: str,
    ...
    Единственное место, знающее имена зоны C: `projects.project_dir(s, settings,
    project_id)` и строка `projects.Project` с полями `workspace_id` и
    ...
    # квота считается на того, кто проект завёл (§2), и в общем пространстве
    # это разные люди.
...
def bytes_used(s: Session, settings: Settings, owner_id: str) -> int:
    """Сколько байт уже занял этот владелец. Считает C, по каталогам тома (§2).
