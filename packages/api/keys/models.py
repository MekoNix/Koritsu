...
**Внешний ключ на `users.id` с `ON DELETE CASCADE`.** В ночь параллельной работы
его здесь не было намеренно: таблицу `users` заводил агент B, и объявленный
отсюда `ForeignKey` означал бы, что мои тесты требуют его таблицы, а его
миграция — моей. После слияния ветвей ключ поставлен (миграция
`c3a71f0d94e6_model_keys_user_fk`), и поставлен он не для порядка: §7 обещает при
удалении аккаунта полное удаление всего, что с ним связано, а без каскада
...
class ModelKey(Row):
    ...
    # `CASCADE` — обещание §7: удалили аккаунт, значит и ключей его нет.
    ...
    # Последние четыре знака — единственное, что видит человек (§3).
