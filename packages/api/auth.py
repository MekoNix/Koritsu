...
Решение владельца §1: **один код обработчиков и два входа**. Значит, у службы
обязано быть одно место, где «чей это запрос» отвечается для обоих входов
сразу, — иначе у каждого обработчика окажется по две ветки, и разойдутся они
молча. Это место здесь.
...
Тремя ступенями, а не одной, потому что маршруты пишут четыре агента: тот, кто
правит `jobs/routes.py`, объявит право там, где смотрит, а тот, кто вешает
...
**Куда с ключом нельзя** (§11 дословно: «Аккаунты и ключи моделей через токен
не даём»): `/api/auth`, `/api/keys`, `/api/tokens`, `/api/admin`. Отказ там —
...
# Зависимость «кто вошёл» берётся у агента B и не заводится второй раз: подмена
...
# Входы, куда с ключом нельзя. Списком путей, а не флагом на роутере: роутеры
# пишут четверо, а список закрытых входов — решение владельца, и читаться оно
# должно в одном месте целиком.
...
class Principal:
    ...
    user: object                       # `accounts.models.User` агента B
...
СКОПЫ: dict[str, str] = {
    # Загрузка материала — своё право (§11): ключ CI, который льёт файлы, не
    # обязан уметь переименовывать проекты.
    "upload_material": MATERIALS_WRITE,
    # Постановка задания — это и есть «запуск прогонов» (§7).
...
def внешний_вход(request: Request, s: SessionDep) -> Principal:
    ...
    идёт мимо Anubis (§11) и не проверяет CSRF: пускать туда сессию сайта
...
def current_principal(request: Request, user: CurrentUser) -> Principal:
    ...
    `CurrentUser` агента B. Эта зависимость — для тех мест, которым важно, чем
    подтверждён запрос.
    ...
    # своём браузере (§11: права — у ключа).
...
def deny_bearer(request: Request) -> None:
    """Сюда ключом нельзя — только сессией сайта (§11).

    Проверяется по пути (`ТОЛЬКО_СЕССИЯ`), а не по флагу на роутере, и зовётся
    из одного места — разбора ключа. Флаг пришлось бы ставить на четыре
    роутера, которые пишут четыре агента, и пятый, заведённый под тем же
    префиксом, оказался бы без него молча. Путь же не забывается: он в списке
