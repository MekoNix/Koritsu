...
# Под каким именем решение помнит общие файлы работы, снятые с него человеком.
# Записью решения, а не пометкой в указателе материалов: указатель отвечает на
# вопрос «чей это файл», а здесь ответ на другой — «что этому решению
# показывать», и один и тот же файл бывает снят у одного решения и оставлен у
# соседнего.
#
# Хранятся именно **снятые**, а не выбранные. Общий файл работы виден каждому
# решению по умолчанию, и список выбранных пришлось бы дописывать при каждой
# новой загрузке: файл, положенный в работу завтра, не попал бы в промпт вовсе
# и молча. Снятие же — это решение про конкретный файл, и про завтрашний оно
# ничего не говорит.
COMMON_OFF_STATE = "kadai-общие"
...
class Project:
    ...
    **Мутирует только `set_value`.** Через неё идут уровень 1, уровень 2,
    ...
    def excluded_common(self) -> list[str]:
        """Общие файлы работы, снятые с этого решения. Пусто — видны все.

        Хранится снятое, а не выбранное (`COMMON_OFF_STATE`): файл, который
        положат в работу завтра, обязан оказаться в промпте сам, иначе выбор
        «показывать общие файлы» пришлось бы подтверждать после каждой загрузки.
        """
        if not self.solution:
            return []
        сырое = self.state(COMMON_OFF_STATE).get("excluded") or ()
        return [str(mid) for mid in сырое]

    def set_excluded_common(self, ids) -> list[str]:
        """Снять с решения названные общие файлы. Список заменяется целиком.
        ...
        который нельзя вернуть обратно.

        У работы, открытой без решения, снимать нечего: общие файлы — это
        файлы этой самой работы, и она видит их все.
        """
        if not self.solution:
            raise OrchestratorError(
                "общие файлы снимаются у решения, а не у работы целиком")
        снятые = sorted({str(mid) for mid in (ids or ())})
        self.put_state(COMMON_OFF_STATE, {"excluded": снятые})
        return снятые

    def context_ids(self) -> set | None:
        """Какие материалы видит модель этого решения. `None` — все, как раньше.

        Три источника: файлы папки самого решения, условие и общие файлы
        работы, с которых человек не снял галочку.

        Условие входит сюда всегда: без него решать нечего, а приписано оно
        бывает и общим файлом работы.

        Общие файлы работы входят по умолчанию: методичка кафедры и требования
        к оформлению кладутся один раз на всю работу, и заставлять человека
        прикладывать их к каждой задаче значило бы хранить один файл столько
        раз, сколько в работе задач. Снятые с решения (`excluded_common`) не
        входят: файл соседней темы сбивает модель ровно так же, как чужое
        условие, и платит за это человек.
        """
        if not self.solution:
            return None
        свои = set(self.solution_materials())
        снятые = set(self.excluded_common())
        свои.update(mid for mid in self.common_materials() if mid not in снятые)
    ...
    def set_condition(self, material_id: str) -> None:
        ...
        **после** приёма файла: до подтверждения условие есть материал, а не
        ...
        if self.solution:
            ...
            self.put_state(CONDITION_STATE, {"material": str(material_id)})
            return
        settings = self.settings()
