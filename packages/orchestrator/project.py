...
# Под каким именем решение помнит свой материал-условие. У работы без решений
# условие по-прежнему лежит в настройках проекта: там оно одно на весь каталог,
# а здесь их столько, сколько решений.
#
# В записи два поля: `material` — условие сейчас, `past` — материалы, бывшие
# условием до него. Прежние остаются в папке решения (по ним видно, что было
# прочитано), но в промпт не едут: два условия сразу, старое и новое, дают
# решение по тому, которое человек как раз исправил.
...
class Project:
    ...
    **Мутирует только `set_value`.** Через неё идут уровень 1, уровень 2,
    ...
    def context_ids(self) -> set | None:
        ...
        Прежние условия (`past_conditions`) не входят никогда, даже лёжа в
        папке решения: правка условия кладётся новым материалом, а старый файл
        остаётся памятью о прочитанном — показать модели оба значило бы решать
        задачу по тексту, который человек как раз исправил.
        """
        if not self.solution:
            return None
    ...
    def set_condition(self, material_id: str) -> None:
        ...
        **после** приёма файла: до подтверждения условие есть материал, а не
        ...
        Прежнее условие решения запоминается (`past_conditions`) и с этой
        минуты модели не показывается. Поправленное условие — это по построению
        другой материал (материал адресуется содержимым), а файл с прежним
        текстом остаётся в папке решения: по нему видно, что было прочитано.
        Оставить его в промпте значило бы показать модели два условия сразу и
        заплатить за решение по тому, которое человек как раз исправил.
        """
    ...
    def past_conditions(self) -> list[str]:
        ...
        Нужны двоим: набору материалов для модели (`context_ids`), куда они не
        входят, и экрану решения, который помечает их в папке — файл лежит на
        месте, но задачу по нему больше не решают.
        """
        if not self.solution:
            return []
