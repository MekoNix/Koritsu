...
Раскладка на диске:
    ...
    <path>/kadai/<решение>/…       то же самое, но своё у каждого решения работы
    <path>/kadai/materials.json    какому решению принадлежит какой материал
    ...
    <path>/reports/<id>/           отдельный документ работы: свой project.json,
                                   manifest.json, values/, blocks/, out/

**Документов в работе много, а материалы и артефакты одни.** Работа переживает
несколько отчётов по разным бланкам (титульный лист, приложение, вторая глава),
и у каждого свои значения тегов, своя история версий и свои сборки. Поэтому
каталог документа отделён от каталога работы: `Project(path, report=<id>)`
работает теми же методами, но читает и пишет в `reports/<id>/`. Материалы,
артефакты, состояния, журнал расхода и потолок остаются общими — файл, принесённый
человеком, принадлежит работе, а не одному её документу.

`report=` без своего каталога означает документ в корне работы: так выглядят
работы, заведённые до появления второго документа, и терять их значения из-за
переезда раскладки нельзя.
...
# Каталог, в котором лежат решения работы: `kadai/<решение>/`. Одно имя, а не
# склейка по месту, потому что читают его трое — раскладка решения, указатель
# материалов и опись решений на диске.
KADAI_DIR = "kadai"

# Указатель «материал → решения, которым он принадлежит». Лежит рядом с
# решениями, а не внутри одного из них: материал бывает общим для всей работы
# (тогда его нет в указателе вовсе) и бывает приложен к двум решениям сразу —
# файл адресуется содержимым, и тот же файл, приложенный дважды, это один
# материал. Список, а не одно имя, ровно поэтому.
KADAI_MATERIALS = "materials.json"

# Под каким именем решение помнит своё условие. У работы без решений условие
# по-прежнему лежит в настройках проекта: там оно одно на весь каталог, а здесь
# их столько, сколько решений.
#
# В записи два поля: `text` — условие задачи словами, `material` — файл, из
# которого его прочитали, если условие принесли файлом.
#
# **Текст старше файла.** Условие, набранное в форме, — это текст и ничего
# больше: заворачивать его в `.txt` и класть материалом значило бы, что правка
# условия — это новый файл, а прежний остаётся в папке решения памятником. Файл
# (pdf, docx, скан) остаётся материалом: по нему видно, что читали, и в нём
# бывают картинки. Но подтверждённый человеком текст ложится сюда же, и решают
# задачу по нему.
#
# Поле `past` пишут записи, заведённые до этого правила: там правка условия
# была новым материалом, и прежние перечислены, чтобы не уехать в промпт вместе
# с нынешним. Читается оно по-прежнему (`past_conditions`), а пишется только
# при смене одного файла-условия на другой.
CONDITION_STATE = "условие"

# Под каким именем решение помнит общие файлы работы, подключённые к нему
# человеком. Записью решения, а не пометкой в указателе материалов: указатель
# отвечает на вопрос «чей это файл», а здесь ответ на другой — «что этому
# решению показывать», и один и тот же файл бывает подключён к одному решению
# и не нужен соседнему.
#
# Хранится именно **подключённое**. Общий файл работы к отдельной задаче
# отношения может не иметь вовсе, а за лишний кусок промпта платит человек:
# файл, положенный в работу, уезжает к модели тогда, когда его к задаче
# подключили, и никогда — молча. Имя записи осталось прежним: под ним лежат и
# записи решений, заведённых до этого правила (`included_common` читает обе
# формы).
COMMON_STATE = "kadai-общие"

# Записи состояния, которые до появления второго решения лежали в корне работы:
# ход стадий, задание и пожелания (`kadai.status.STATE_KEY`/`TASK_KEY`,
# `orchestrator.kadai.WISHES_KEY`). Список здесь копией, а не импортом: сценарий
# про проект не знает, а оркестратор не имеет права импортировать надстройку.
_КОРНЕВЫЕ_ЗАПИСИ = ("kadai", "kadai-задание", "kadai-пожелания")

# Форма имени решения. Оно становится звеном пути, поэтому проверяется до
# первого `join`, а не подчищается: подрезанное имя означало бы запись не в тот
# каталог, о которой никто не узнал.
_RUN_RE = re.compile(r"\A[A-Za-z0-9][A-Za-z0-9_.-]{0,63}\Z")
...
# Каталог, под которым лежат отдельные документы работы. Имя закреплено
# константой, потому что читают его двое: сам проект и выгрузка, складывающая
# значения каждого документа в архив, — а разошедшееся имя означало бы архив без
# половины работы.
...
# здесь нет намеренно: как зовут отчёт, помнит журнал запусков, а `name` в
# настройках — имя самой работы, и оно уезжает в собранный документ.
НАСТРОЙКИ_ДОКУМЕНТА = ("template", "template_source", "created")
...
def _имя_решения(solution) -> str:
    """Имя решения как звено пути — или отказ. Формой, а не подчисткой.

    Имя приходит от службы (идентификатор записи журнала запусков), но путь
    складывается здесь, и проверять его обязан тот, кто складывает: подрезанное
    имя означало бы запись в чужой каталог, о которой никто не узнал.
    """
    имя = str(solution or "")
    if not _RUN_RE.match(имя):
        raise OrchestratorError(
            f"имя решения {solution!r} не годится звеном пути: буквы латиницей, цифры, "
            "«.», «-» и «_», не длиннее 64 знаков")
    return имя
...
class Project:
    ...
    **Мутирует только `set_value`.** Через неё идут уровень 1, уровень 2,
    ...
    def __init__(self, path: str, *, report: str = "", solution: str = ""):
        """Каталог работы, а `report` — какой её документ открыт.

        Пустой `report` — единственный документ работы, лежащий в корне
        каталога; так работа выглядела, пока документ у неё был один, и так же
        её читают модули, которым документов не нужно вовсе (материалы, схемы).

        Названный `report`, у которого каталога ещё нет, открывает тот же
        корневой документ. Это не молчаливая ошибка, а перевод старых работ:
        запись журнала о первом отчёте появилась раньше, чем каталоги
        документов, и отвечать на неё пустым набором значений значило бы
        потерять написанное.

        `solution` — то же самое для решения: в работе их тоже много, и у каждого
        свой ход стадий, своё условие, свои пожелания и свой список блоков с
        историей. Разница с `report` одна — каталог (`kadai/<решение>/`) и то,
        что подмены на корень у решения нет: каталог решения заводится вместе с
        самим решением, и пустой ход стадий у только что заведённого — верный
        ответ, а не потеря.
        """
        self.path = os.path.abspath(path)
        if not os.path.isdir(self.path):
            raise OrchestratorError(f"каталога проекта нет: {self.path}")
        self.report = _safe_leaf(report, "имя документа") if report else ""
        self.solution = _имя_решения(solution) if solution else ""
    ...
    # ── документы работы ────────────────────────────────────────────────────
    # Работа держит несколько документов: у каждого свой бланк, свои значения с
    # историей и свои сборки. Материалы, артефакты и учёт расхода при этом одни
    # на работу — файл, принесённый человеком, не принадлежит одному отчёту.

    def reports(self) -> list[str]:
        """Документы работы, у которых есть свой каталог, по возрастанию имени.

        Документ в корне (работа, заведённая до появления второго) сюда не
        попадает: у него нет имени, под которым его можно было бы назвать, —
        как он зовётся, помнит журнал запусков.
        """
        корень = os.path.join(self.path, ПАПКА_ДОКУМЕНТОВ)
    ...
    def create_report(self, report: str, *,
        ...
        материалы, журнал, потолок расхода, — а здесь заводится только документ,
        и второй набор материалов рядом с первым означал бы файлы, которые видно
        из одного отчёта и не видно из другого.

        `template=None` — документ без бланка, как и у работы: `hokoku` строит
        пустой DOCX со стилями, а `template_source` помнит, что документ наш и
        первый же принесённый бланк может заменить его молча.

        Поверх существующего документа — отказ: «завести» и «сменить бланк» —
        разные намерения, и второе делает `update_template`.
        """
        имя = _safe_leaf(report, "имя документа")
        каталог = os.path.join(self.path, ПАПКА_ДОКУМЕНТОВ, имя)
        if os.path.isdir(каталог):
            raise OrchestratorError(f"документ {имя!r} в работе уже есть")
        свой = template is None
        if свой:
            template = hokoku.document_bytes(hokoku.blank_document())
        elif not isinstance(template, (bytes, bytearray)):
        ...
        документ._write_json(документ._settings_path(), {
            "template": art, "template_source": "blank" if свой else "given",
            "created": _now()})
        документ.save_manifest(hokoku.manifest_from_template(bytes(template)))
        return документ

    def drop_report(self, report: str) -> bool:
        """Снести документ работы: значения, их версии, блоки, каталог сборки.

        Артефакты не трогаются, и это то же правило, что у записи журнала
        запусков: артефакт адресуется содержимым, тот же файл бывает значением
        тега в соседнем отчёте, и снести его вслед за документом значило бы
        выбить картинку из чужого готового документа. Место освобождает уборка
        работы целиком.

        Документа нет — `False`, а не отказ: «удалить то, чего нет» приводит
        ровно к тому состоянию, которого просили.
        """
        имя = _safe_leaf(report, "имя документа")
        каталог = os.path.join(self.path, ПАПКА_ДОКУМЕНТОВ, имя)
    ...
    # ── решения работы ──────────────────────────────────────────────────────
    # Решений в работе столько же, сколько отчётов, и по той же причине: одна
    # задача — одно решение, а задач в работе несколько. У каждого свои стадии,
    # своё условие, свои пожелания, свой список блоков с версиями и свои
    # готовые файлы. Общими остаются материалы, артефакты и учёт расхода — файл
    # адресуется содержимым, и вторая его копия под вторым решением была бы тем
    # же файлом, занявшим место дважды.

    def for_solution(self, solution: str = "") -> "Project":
        """Тот же каталог работы, но глазами одного решения. Пусто — общий вид.

        Заводить решение отдельным классом незачем: методы те же, меняется один
        каталог. А вот подставлять его аргументом у каждого вызова
        (`state(name, run=…)`) нельзя: сценарий (`kadai`) зовёт у проекта два
        десятка методов и про решения не знает вовсе — аргумент пришлось бы
        протаскивать через все двери и через сам сценарий, то есть научить его
        тому, чего он знать не должен.

        Журнал расхода передаётся тем же объектом, а не заводится вторым:
        `Limit.spent` считает по записям своего журнала, и второй экземпляр не
        увидел бы того, что записал первый, — потолок перестал бы работать
        ровно во время прогона.
        """
        вид = Project(self.path, solution=solution)
        вид._journal = self._journal
        return вид

    def create_solution(self, solution: str) -> "Project":
        """Завести каталог решения. Повторно на том же — то же состояние.

        Каталог заводится сразу, а не при первой записи, ровно затем, чтобы
        «решение есть, но ход стадий пуст» и «решения нет» были разными
        состояниями на диске: у второго решения работы иначе нельзя отличить
        «ещё не запускали» от «читаем чужое».
        """
        каталог = os.path.join(self.path, KADAI_DIR, _имя_решения(solution))
        os.makedirs(каталог, exist_ok=True)
        return self.for_solution(solution)

    def solutions(self) -> list[str]:
        """Имена решений, у которых есть каталог, по возрастанию. Пусто — их нет.

        Нужно выгрузке (`api/export`): архив уносит работу целиком, а «целиком»
        с появлением второго решения значит «все решения», и обойти их без
        описи нечем.
        """
        корень = os.path.join(self.path, KADAI_DIR)
        if not os.path.isdir(корень):
            return []
        return sorted(имя for имя in os.listdir(корень)
                      if _RUN_RE.match(имя)
                      and os.path.isdir(os.path.join(корень, имя)))

    def drop_solution(self, solution: str) -> bool:
        """Снести решение: ход стадий, условие, пожелания, блоки, готовые файлы.

        Материалы и артефакты остаются, и это то же правило, что у документа:
        артефакт адресуется содержимым и бывает вставлен в соседнюю работу, а
        материал принадлежит работе. Приписанные решению файлы снимает с учёта
        тот, кто решает их судьбу (`unbind_material`), — здесь про них не
        известно, ссылается ли на них кто-то ещё.

        Решения нет — `False`, а не отказ: просили состояние, в котором его
        нет, и оно уже наступило.
        """
        каталог = os.path.join(self.path, KADAI_DIR, _имя_решения(solution))
        if not os.path.isdir(каталог):
            return False
    ...
    def adopt_root_kadai(self, solution: str) -> bool:
        """Перенести решение из корня работы в свой каталог. → переносили ли.

        Так работа выглядела, пока решение в ней было одно: ход стадий, задание,
        пожелания и список блоков лежали в корне каталога. Второе решение в той
        же работе читало бы их и затирало, поэтому первое переезжает в свой
        каталог — целиком, вместе с историей версий блоков.

        Переносится ровно то, что принадлежит решению. Каталог сборки (`out/`)
        остаётся на месте намеренно: собранное уже лежит артефактами, а
        идентификаторы их записаны в задании, которое переезжает, — новая сборка
        просто положит файлы в каталог решения.

        Условие переезжает из настроек работы в запись состояния решения: в
        настройках оно одно на каталог, и второе решение стёрло бы первое.

        Ничего не переносилось (решений в корне нет или каталог уже занят) —
        `False`. Повторный вызов безопасен: он и есть обычный ход, потому что
        зовётся этот метод на каждом чтении списка решений.
        """
        имя = _имя_решения(solution)
        свой = os.path.join(self.path, KADAI_DIR, имя)
        переехало = False
        for запись in _КОРНЕВЫЕ_ЗАПИСИ:
            откуда = os.path.join(self.path, "state",
                                  f"{_slug(hokoku.wire.norm_key(запись))}.json")
            куда = os.path.join(свой, "state",
                                f"{_slug(hokoku.wire.norm_key(запись))}.json")
            if os.path.isfile(откуда) and not os.path.exists(куда):
                os.makedirs(os.path.dirname(куда), exist_ok=True)
                os.replace(откуда, куда)
                переехало = True
        корневые_блоки = os.path.join(self.path, "blocks")
        свои_блоки = os.path.join(свой, "blocks")
        if os.path.isdir(корневые_блоки) and not os.path.isdir(свои_блоки):
            os.makedirs(свой, exist_ok=True)
            os.replace(корневые_блоки, свои_блоки)
            переехало = True
        условие = self._read_json(os.path.join(self.path, "project.json"), {}) \
            .get("condition")
        if переехало and условие:
            вид = self.for_solution(имя)
            if not вид.condition():
                вид.set_condition(str(условие))
        return переехало

    # ── чей материал: работы или одного решения ─────────────────────────────
    # Файлы контекста принадлежат решению, а не работе: методичка одной задачи
    # в промпте другой сбивает модель ровно так же, как чужое условие, и платит
    # за это человек. Указатель отдельным файлом, а не пометкой в самом
    # материале, по двум причинам: материал адресуется содержимым (тот же файл,
    # приложенный к двум решениям, — один материал и обе привязки сразу), а
    # хранилище материалов про решения не знает и знать не должно.
    #
    # Файл, приложенный ко всей работе (в указателе его нет вовсе), — общий: он
    # виден каждому решению, пока с него не сняли галочку. Так лежат методичка
    # кафедры и требования к оформлению — их кладут один раз на работу, а нужны
    # они в каждой задаче.

    def material_solutions(self) -> dict:
        """Весь указатель: материал → решения, которым он приписан. Копия с диска.

        Материала в указателе нет — он общий для работы: так лежат файлы,
        приложенные ко всей работе, и отчёты видят их как раньше.
        """
        сырое = self._read_json(self._kadai_materials_path(), {}) or {}
        return {str(k): [str(r) for r in (v or ())] for k, v in сырое.items()}

    def bind_material(self, material_id: str, solution: str = "") -> None:
        """Приписать материал решению. Приписанный второй раз — то же состояние.

        Решение по умолчанию — то, которым открыта работа: файл, положенный на
        странице решения, принадлежит ему, и называть решение второй раз
        вызывающему незачем.
        """
        имя = _имя_решения(solution or self.solution)
        указатель = self.material_solutions()
        свои = указатель.setdefault(str(material_id), [])
        if имя in свои:
            return
        свои.append(имя)
        self._write_json(self._kadai_materials_path(), указатель)

    def unbind_material(self, material_id: str, solution: str = "") -> None:
        """Снять приписку материала. Без имени решения — от всех сразу.

        Сам материал остаётся: он принадлежит работе, и удалять его — отдельное
        действие с отдельной ценой ошибки (на файл может ссылаться значение
        тега в соседнем документе).
        """
        указатель = self.material_solutions()
        mid = str(material_id)
        if mid not in указатель:
            return
        if solution:
            указатель[mid] = [r for r in указатель[mid] if r != str(solution)]
        if not solution or not указатель[mid]:
            указатель.pop(mid)
        self._write_json(self._kadai_materials_path(), указатель)

    def solution_materials(self, solution: str = "") -> list[str]:
        """Материалы, приписанные решению. Без общих файлов работы."""
        имя = str(solution or self.solution or "")
        if not имя:
            return []
        return sorted(mid for mid, свои in self.material_solutions().items()
                      if имя in свои)

    def common_materials(self) -> list[str]:
        """Материалы работы, не приписанные ни одному решению: общие для всех."""
        занятые = set(self.material_solutions())
        return [m.id for m in self.store().list() if m.id not in занятые]

    def included_common(self) -> list[str]:
        """Общие файлы работы, подключённые к этому решению. Пусто — ни одного.

        Хранится подключённое, а не снятое (`COMMON_STATE`): общий файл работы
        к отдельной задаче отношения может не иметь вовсе, и уезжать в её
        промпт молча он не должен — за лишний кусок промпта платит человек, а
        чужая методичка сбивает модель так же, как чужое условие.

        Читаются обе формы записи. Решения, заведённые до этого правила, помнят
        **снятое** (`excluded`), и подключённым у них считается всё остальное:
        человек однажды выбрал, что показывать, и менять этот выбор задним
        числом нельзя — прогон стоил бы других денег и дал бы другую работу.
        """
        if not self.solution:
            return []
        запись = self.state(COMMON_STATE)
        if "included" in запись:
            return [str(mid) for mid in (запись.get("included") or ())]
        if "excluded" in запись:
            снятые = {str(mid) for mid in (запись.get("excluded") or ())}
            return [mid for mid in self.common_materials() if mid not in снятые]
        return []

    def set_included_common(self, ids) -> list[str]:
        """Подключить к решению названные общие файлы. Список заменяется целиком.

        Целиком, а не по одному: человек ставит галочки списком и присылает то,
        что видит на экране, — сложение с прежним состоянием означало бы файл,
        который нельзя отключить обратно.

        Запись заменяет и прежнюю форму (`excluded`): раз человек выбрал
        заново, старому выбору браться неоткуда.

        У работы, открытой без решения, подключать нечего: общие файлы — это
        файлы этой самой работы, и она видит их все.
        """
        if not self.solution:
            raise OrchestratorError(
                "общие файлы подключаются к решению, а не к работе целиком")
        выбранные = sorted({str(mid) for mid in (ids or ())})
        self.put_state(COMMON_STATE, {"included": выбранные})
        return выбранные

    def context_ids(self) -> set | None:
        """Какие материалы видит модель этого решения. `None` — все, как раньше.

        Три источника: файлы папки самого решения, условие и те общие файлы
        работы, которые человек к решению подключил.

        Условие входит сюда всегда: без него решать нечего, а приписано оно
        бывает и общим файлом работы.

        Общие файлы работы по умолчанию не входят: методичка кафедры нужна не
        каждой задаче работы, а промпт, в который она уехала сама, человек
        оплачивает, не увидев. Подключаются они поштучно
        (`set_included_common`).

        Условие, набранное словами, материалом не становится вовсе
        (`set_condition_text`): в набор ему попадать нечем, и в промпт оно едет
        отдельным куском, как текст, а не как приложенный файл.

        Прежние условия (`past_conditions`) не входят никогда, даже лёжа в папке
        решения: файл, заменённый другим файлом, остаётся памятью о прочитанном
        — показать модели оба значило бы решать задачу по тексту, который
        человек как раз исправил.
        """
        if not self.solution:
            return None
        свои = set(self.solution_materials())
        общие = set(self.common_materials())
        свои.update(mid for mid in self.included_common() if mid in общие)
        свои.difference_update(self.past_conditions())
        условие = self.condition()
        if условие:
            свои.add(условие)
        return свои
    ...
    def settings(self) -> dict:
        ...
        У открытого документа поверх настроек работы ложатся его собственные
        (`НАСТРОЙКИ_ДОКУМЕНТА`). Разрез именно такой: бланк принадлежит
        документу — их в работе несколько и они разные, — а потолок расхода и
        пресет по умолчанию принадлежат работе, и вторая их копия в каждом
        документе разошлась бы с первой при первой же смене потолка.
        """
        общие = self._read_json(os.path.join(self.path, "project.json"), {})
        свои = self._settings_path()
        if свои != os.path.join(self.path, "project.json"):
            общие.update(self._read_json(свои, {}))
        return общие

    def save_settings(self, d: dict) -> None:
        """Настройки на диск. У документа записываются только его собственные.

        Отбор по `НАСТРОЙКИ_ДОКУМЕНТА`, а не запись словаря целиком: `settings()`
        отдаёт слитые настройки, и записать их в файл документа значило бы
        сделать снимок потолка работы, который перестанет меняться вместе с ней.
        """
        if self._settings_path() != os.path.join(self.path, "project.json"):
            d = {k: v for k, v in d.items() if k in НАСТРОЙКИ_ДОКУМЕНТА}
    ...
    def add_material(self, data: bytes, name: str, *, do_ocr: bool = True,
        ...
        if self.solution:
            # Файл, положенный в открытое решение, принадлежит ему: так же
            # ложится и исходник, сочинённый моделью (`live.put_source`).
            self.bind_material(material.id)
        if condition:
    ...
    def set_condition(self, material_id: str) -> None:
        ...
        **после** приёма файла: до подтверждения условие есть материал, а не
        ...
        Подтверждённый текст условия при смене файла стирается: он принадлежал
        прежнему файлу, и оставить его значило бы решать новую задачу по старому
        тексту. Заново он появится, когда стадия «приём» прочитает новый файл, а
        человек подтвердит или поправит прочитанное (`set_condition_text`).

        Прежнее условие решения запоминается (`past_conditions`) и с этой минуты
        модели не показывается: файл с прежним текстом остаётся в папке решения,
        и оставить его в промпте значило бы показать модели два условия сразу.
        """
        self.store().get(str(material_id))                 # нет такого — ошибка materials
        if self.solution:
            # У решения условие своё, и лежит оно записью состояния решения, а
            # не в настройках: настройки одни на работу, и второе решение
            # затирало бы условие первого молча.
            новое = str(material_id)
            запись = self.state(CONDITION_STATE)
            было = str(запись.get("material") or "")
            прежние = [str(mid) for mid in (запись.get("past") or ())]
            if было and было != новое and было not in прежние:
                прежние.append(было)
            текст = "" if было != новое else str(запись.get("text") or "")
            # Названный заново старый файл — снова условие, и «прежним» ему быть
            # уже нельзя: иначе он не доехал бы до модели и в этот раз.
            self.put_state(CONDITION_STATE,
                           {"text": текст, "material": новое,
                            "past": [mid for mid in прежние if mid != новое]})
            return
        settings = self.settings()
        было = str(settings.get("condition") or "")
        settings["condition"] = str(material_id)
        self.save_settings(settings)
        запись = dict(self.state(CONDITION_STATE))
        if было != str(material_id) and запись.get("text"):
            запись["text"] = ""
            self.put_state(CONDITION_STATE, запись)

    def set_condition_text(self, text: str) -> str:
        """Условие задачи словами. → записанный текст.

        Это и есть подтверждённое условие: набранное в форме, поправленное после
        распознавания скана или переписанное человеком, передумавшим по дороге.
        Текстом, а не файлом, по одной причине — правка. Условие, завёрнутое в
        `.txt` и положенное материалом, правится только вторым файлом: первый
        остаётся в папке решения, требует пометки «прежнее» и объяснения, почему
        его показывают, но по нему не решают. Текста это не касается вовсе: он
        заменяется на месте, и в решении его ровно один.

        Файл-условие при этом не трогается: он назван отдельно
        (`set_condition`), и по нему видно, из чего текст взят. Стереть текст —
        передать пустую строку; тогда условие снова читается из файла.
        """
        запись = dict(self.state(CONDITION_STATE))
        запись["text"] = str(text or "")
        self.put_state(CONDITION_STATE, запись)
        return запись["text"]

    def condition_text(self) -> str:
        """Условие задачи словами. Пусто — его не набирали и не подтверждали.

        Пусто не значит «условия нет»: оно бывает названо файлом, и текст из
        него читает стадия «приём». Значит это ровно одно: словами условие ещё
        не набирали и прочитанное из файла не подтверждали.
        """
        return str(self.state(CONDITION_STATE).get("text") or "")

    def condition(self) -> str | None:
        """Идентификатор материала-условия или None, если его не называли."""
        if self.solution:
            return str(self.state(CONDITION_STATE).get("material") or "") or None
        return self.settings().get("condition") or None

    def past_conditions(self) -> list[str]:
        """Материалы, бывшие условием этого решения до нынешнего. По порядку.

        Нужны набору материалов для модели (`context_ids`), куда они не входят.
        Условие правится текстом, поэтому расти этот список перестал: в него
        попадает только файл, заменённый другим файлом, да записи решений,
        заведённых до правки условия текстом.
        """
        if not self.solution:
            return []
    ...
    def doc_path(self) -> str:
        """Каталог открытого документа: `reports/<id>` или сам корень работы.

        Одно место, где решается, куда легли значения этого документа. Каталога
        нет — работаем с корнем: см. докстроку `__init__` про старые работы.
        """
        if self.solution:
            # У решения подмены на корень нет намеренно: каталог заводится
            # вместе с самим решением, и пустой ход стадий у только что
            # заведённого — верный ответ, а не потеря. Подставь мы корень, второе
            # решение работы читало бы ход первого.
            return os.path.join(self.path, KADAI_DIR, self.solution)
        if not self.report:
            return self.path
        свой = os.path.join(self.path, ПАПКА_ДОКУМЕНТОВ, self.report)
        return свой if os.path.isdir(свой) else self.path

    def _settings_path(self) -> str:
        # У решения своего бланка и своих настроек нет: потолок расхода, пресет
        # и имя одни на работу, а документ решения собирается из списка блоков,
        # то есть шаблона у него не бывает вовсе.
        return os.path.join(self.path if self.solution else self.doc_path(),
                            "project.json")
    ...
    def _values_dir(self) -> str:
        return os.path.join(self.doc_path(), "values")

    def _runs_dir(self) -> str:
        return os.path.join(self.path, "runs")

    def _blocks_dir(self) -> str:
        return os.path.join(self.doc_path(), "blocks")
    ...
    def _state_path(self, name: str) -> str:
        ...
        Каталог — открытого решения, если работа открыта решением: ход стадий,
        задание и пожелания у каждого свои. Состояния самой работы (опись
        выгрузок, например) пишутся общим видом и лежат в корне.
        """
        return os.path.join(self.doc_path(), "state",
                            f"{_slug(hokoku.wire.norm_key(str(name)))}.json")

    def _kadai_materials_path(self) -> str:
        """Указатель «материал → решения». Один на работу, рядом с решениями."""
        return os.path.join(self.path, KADAI_DIR, KADAI_MATERIALS)
    ...
    def outdir(self) -> str:
        ...
        path = os.path.join(self.doc_path(), "out")
        os.makedirs(path, exist_ok=True)
        return path
...
__all__ = ["Project", "Version", "BlockVersion", "Run", "SOURCES", "BLOCK_FIELDS",
           "artifact_id", "template_tags", "KADAI_DIR", "CONDITION_STATE",
           "COMMON_STATE"]
