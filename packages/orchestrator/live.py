...
Второй способ работы: шаблона нет вовсе, работа — это упорядоченный список
именованных блоков, документ — его сборка. Сам список и всё, что с ним делают,
живёт в `hokoku.live` (`Work`, `Block`, `insert`/`replace`/ `remove`/`move`,
объявления инструментов и `call_tool`, `texts_schema`, `fill_texts`, `assemble`).
Здесь — то, чего у чистого движка отчётов нет и быть не должно: диск, версии,
`source`, вызов модели, журнал расхода и лимит.
...
Связный текст петлёй **не пишется**: скелет и нетекстовое собирает `solve`, весь
текст пишется одним проходом `write_texts` по готовому списку, видя соседей.
Абзацы, написанные по одному, разойдутся между собой, и увидит это тот, кто
прочтёт работу целиком.
"""
...
# унаследованное от `llm.Limits` (12): там оно про один вопрос с
# инструментами, а здесь агент собирает работу целиком — заголовки, код,
# таблицы, схемы, — и дюжины ходов на это не хватает. Потолок нужен всё
# равно: петля, которая вставляет и вставляет, иначе платит вечно.
MAX_STEPS = 50
...
class LiveBox(tools_mod.ToolBox):
    ...
    def _block_tool(self, args: dict) -> dict:
        ...
        if len(work) > потолок:
            # Потолок блоков один на службу и на движок отчётов: свой, вдвое
            # меньший, означал бы, что список из 700 блоков петлёй не собрать,
            # а руками — можно, и объяснить эту разницу человеку было бы нечем.
...
def solve(project, task: str, *, endpoint: str, tools=None, chunks=(), data=(),
    ...
    Связный текст здесь не пишется — это отдельный
...
def _solve_request(task: str) -> str:
    ...
    return (f"{str(task).strip()}\n"
            ...
            "(make_flowchart, make_class_diagram, make_object_diagram), графики "
            "по числам рисует make_chart (данные и вид графика — картинка "
            "приходит артефактом, ставь её блоком image с подписью), блоки "
...
def write_texts(project, *, endpoint: str, chunks=(), data=(), max_tokens=None,
    ...
    Довод посчитан заранее: блоки связаны так же, как теги (введение ссылается
    на цель, вывод — на замеры), и текст, написанный по одному блоку в петле,
    будет связным по отдельности и рассогласованным вместе. Плюс расход: петля
    с историей — самый дорогой способ написать абзац.
    ...
    parts.append(Part(role="request", stable=False, text=(
        ...
        "ставит сборщик. Ключ бери из списка блоков и только оттуда: ссылка на "
        "блок, которого в работе нет, оставит в документе «?» вместо номера — "
        "лучше обойтись без ссылки вовсе.")))
    fill_mod._seal(project, run, parts)
