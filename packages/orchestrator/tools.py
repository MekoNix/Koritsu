...
# человеку (наружу только `gost_19_701_90`). Псевдоним `loopLimit`
...
# Вопрос: пускать ли уровень 3 на endpoint, у которого нет операторского канала
# (то есть где наши указания для модели не весомее текста из файла студента).
# Ответ: заглушка, идём дальше. Заглушка стоит здесь одна на весь уровень, а не
...
#
# Заглушка осталась «flag» и после того, как канал начали мерить
# (`llm.probing._step_operator`): у deepseek и openrouter указание оператора устояло
# 2 из 2, заявка поднята до `messages_system`, и на этих двух ворота теперь
...
# измерением — только явным решением в `presets.py`, и отозвать его можно той же
# строкой.
...
def operator_channel_gate(endpoint_id: str) -> tuple[tuple[str, ...], list[hokoku.Problem]]:
    ...
    сдерживание не равно каналу, и утверждать обратное на основании одной
    заявки нельзя.
    """
    ...
    problem = fill_mod._problem(
        ...
        f"({caps.operator_channel!r}); прогон уровня 3 разрешён в режиме "
...
def tools() -> list[llm.Tool]:
    ...
    Имена первых двух — `list_materials` и `read_material`, а не прежние
    `list_project_files` / `read_file`. Смысл не менялся ни на день — **пути не
    принимаются ни в одном аргументе**, — но слова видит модель, и `read_file`
    звало её к файловой системе, которой у неё нет: файлов у нас не
    существует, есть материалы с
