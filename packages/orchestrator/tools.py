...
class ToolBox:
    ...
    def __init__(self, project, run, *, parts, manifest=None, template=None,
        ...
        self._имена: dict = {}
        # Набор материалов решения считается один раз и запоминается:
        # `context_ids` обходит опись на томе, а спрашивают его у каждого чтения
        # файла. Память сбрасывает тот, кто кладёт материал посреди прогона
        # (`забыть_контекст`).
    ...
    def контекст(self) -> set | None:
        ...
        Обёртка над `Project.context_ids` с памятью: там на каждый вызов идёт
        обход описи материалов, а зовут его и опись, и каждое чтение файла.

        Память живёт до первого нового материала, а не до конца прогона: набор
        меняется прямо посреди хода модели, когда она кладёт свой исходник
        (`live.LiveBox._put_source`). Запомненный навсегда, он отвечал бы
        «материала в проекте нет» на файл, который сам же прогон только что и
        положил, — и построить схему по своему коду было бы нечем.
        """
    ...
    def забыть_контекст(self) -> None:
        """Набор материалов пересчитается на следующем вопросе.

        Зовёт тот, кто положил материал этому решению: набор он изменил, и
        память о прежнем с этой минуты неверна.
        """
        self._контекст, self._контекст_считан = None, False
