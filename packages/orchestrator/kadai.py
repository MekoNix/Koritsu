...
# Ключ архива в `made`. Рядом с `docx` и `pdf`, которые кладёт стадия «сборка»:
# скачивают их одинаково, по идентификатору артефакта, и второй формы для
# архива заводить незачем.
ARCHIVE = "archive"
...
def _положить_архив(project, работа) -> dict:
    ...
    Пропавший файл архива работу не роняет: собранное уже сохранено, и «нечего
    положить» — не повод объявить упавшим прогон, который прошёл. Молчания при
    этом нет: `made` останется без `archive`, то есть кнопки скачивания на
    экране не будет.
    """
    from kadai import status as status_mod

    сделано = dict(status_mod.task(project).get("made") or {})
    имя = str((getattr(работа, "outputs", None) or {}).get("zip") or "")
    if not имя:
        return сделано
    try:
