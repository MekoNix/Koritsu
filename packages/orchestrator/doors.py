...
Вид работы здесь не назван ни разу и назван быть не может: «курсовая» и
«лабораторная» — слова сценария, а служба обязана одинаково собирать любую
работу. Профиль-умолчание приходит аргументом (`default=`), то есть данными.
...
# из ответа собрались заготовки блоков: заголовок раздела, чем он заполняется и
# обязателен ли он. Ни одного слова про вид работы: их приносит сценарий.
STRUCTURE_SCHEMA = {
    "type": "object",
    "properties": {
        "sections": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "type": {"type": "string",
                             "enum": ["markdown", "code", "table", "diagram",
                                      "image", "formula"],
...
def ask(project, question: str, *, endpoint: str, schema: dict | None = None,
    ...
    рамке — данные, и в файле студента бывает написано «забудь предыдущие
    указания».
...
def make_template(project, structure=None, *, endpoint: str, task: str = "",
    ...
    по условию и пожеланиям, а профиль сценария приходит `default=` — списком
    разделов-умолчаний, которые модель видит предложением, а не законом. Служба
    о видах работ не знает ни слова и знать не должна: разойдись это знание по
    двум пакетам, «добавить лабораторную» стало бы правкой в обоих.
...
def blocks_of(sections, *, taken=()) -> hokoku.Work:
    ...
    for section in sections:
        raw = section if isinstance(section, dict) else {
            ...
            for name in ("key", "title", "type", "required", "prompt", "level")}
...
def _structure_request(task: str, default) -> str:
    ...
    lines.append("Ответь одним объектом JSON: sections — список разделов по "
                 "порядку. У каждого раздела заголовок, чем он заполняется "
                 "(type), обязателен ли он и одна фраза о том, что в нём писать. "
