...
**`ask` — безтеговая дверь к модели**. Обе прежние точки входа привязаны к
тегам: `fill_tag` отказывает на ключе вне манифеста, `fill_report` строит схему
из манифеста. Стадиям «разбор задания» и «структура» отвечать было нечем, и без
этой двери `kadai` пришлось бы импортировать `llm` — то есть завести второй
средний слой. Внутри всё то же самое, что у уровня 1: раскладка кусков, рамка
вокруг недоверенного текста, метка рамки на прогон, журнал расхода, лимит
пользователя и проверка ответа схемой. Не заводится ровно одно — версия тега: у
ответа нет тега.

**`make_template` — структура работы, а не документ**. Модель сочиняет
**строение**: какие разделы, что в них обязательно, чем они заполняются.
Заготовки блоков по этому строению собираются детерминированно, без модели. Вид
работы здесь не назван ни разу и назван быть не может — не называет его и
сценарий: работой может оказаться что угодно, от отчёта о продажах до записки,
и служба обязана собирать любую одинаково. Отсюда пара полей у раздела: `kind`
— как раздел называется в этой работе (свободное слово от модели), `type` — чем
он заполняется (перечень движка отчётов). Умолчания, если вызывающему есть что
предложить, приходят аргументом (`default=`), то есть данными.

**`check_code` — статическая проверка сочинённого кода**. Код в архиве агент
пишет сам, исполнять его мы не будем никогда, и единственное, что можно
честно обещать, — что он разбирается. Это tree-sitter из `kyotsu`, тот же,
которым `fragmos` строит схемы; отдельной двери не было ни у кого, потому
что тем нужен AST, а здесь нужен один ответ: целый ли исходник. Знание о
языках при этом остаётся в службе — сценарию оно не принадлежит.

**`kadai_services` — сборка всех дверей.** Импортировать соседей позволено
только оркестратору, и направление здесь именно такое: `orchestrator` знает про
`kadai`, `kadai` про `orchestrator` — нет. Импорт стоит внутри функции
намеренно: сценарий — надстройка, и `import orchestrator` не должен падать
оттого, что в надстройке опечатка.
"""
...
def make_template(project, structure=None, *, endpoint: str, task: str = "",
    ...
    Вид работы не фиксирован: строение сочиняется по условию и пожеланиям, а
    `default=` — список разделов-предложений, если вызывающему есть что
    предложить; модель видит их предложением, а не законом. Служба о видах работ
    не знает ни слова и знать не должна — их не знает никто: работой бывает что
    угодно, и палитра разделов, записанная где-либо в коде, была бы утверждением
    ...
    выразить: пустое значение в отчёте — ошибка, и заведи мы его здесь, вся
    работа сразу оказалась бы полной битых блоков.
...
def check_code(text, lang: str = "", *, name: str = "") -> list:
    ...
    **Код проверяется статически и никогда не исполняется.** Здесь это ровно
    tree-sitter: грамматика разбирает текст, не запуская его, и находит то, что
    от разбора и ждут, — обрывки и синтаксический мусор. На вопрос «верно ли
    работает решение» она не отвечает, и в архиве это написано дословно
    (`kadai.archive.NOT_RUN`).
    ...
    целый ли исходник. Заводить это знание в `kadai` нельзя — знание о языках
    сценарию не принадлежит, — поэтому дверь тут.
...
def kadai_services(project, *, endpoint: str, **defaults):
    ...
    соседей импортирует только оркестратор.
    """
