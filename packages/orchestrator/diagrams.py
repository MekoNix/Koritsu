...
**Язык называется одним словом на весь проект** — `py`, `cs`, `cpp` (решение
плана ночи 2). Внутри соседи зовут те же языки иначе (`fragmos` ждёт `python` и
`csharp`), и приведение стоит здесь, в одном месте: таблица, размноженная по
вызывающим, — это тот же расход, что и переписанные двадцать строк. Чужие имена
на входе принимаются (`python`, `csharp`, `c++`) — инструменты агента объявляют
модели свой перечень и присылают его дословно, и ломать их ради красоты входа
незачем.
...
# перечне не показываются (решение владельца §12: «наружу только
# `gost_19_701_90`, псевдоним `loopLimit` из списка режимов убрать»).
#
...
def flowchart_modes() -> list[dict]:
    ...
    переводит его сайт (решение владельца §12).
...
def mode(значение: object) -> str:
    ...
    прийти `loopLimit` (он лежал в перечне до §12 и остался в чужих настройках),
