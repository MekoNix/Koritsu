 ...
 *
 * **Почему свой SVG.** Цвет здесь — переменная темы (`var(--accent)`), а тем
 * четыре и у каждой светлый и тёмный вид: `fill="var(--accent)"` перекрашивается
 * сменой атрибута на `<html>` сам, тогда как библиотека графиков берёт цвет
 * строкой в JS и потребовала бы читать `getComputedStyle` и перерисовывать всё
 * на каждое переключение. Плюс вес: три простых графика не стоят зависимости
 * размером с сам сайт, а вся арифметика — сорок строк в `chart.ts`, где она и
 * проверяется тестом.
 *
 ...
 * Пары цветов проверены на различимость при дальтонизме во всех восьми
 * сочетаниях тема × режим (`accent` против `err`).
 *
      ...
      >
        ...
        <g transform={`translate(${ПОЛЯ.left} ${ПОЛЯ.top})`}>
          {ticks.map((деление) => {
            ...
            return (
              <g key={деление}>
                <line x1={0} x2={ОБЛАСТЬ_Ш} y1={y} y2={y} stroke="var(--line)" strokeWidth={1} />
                <text
                  x={-8}
                  y={y + 4}
                  textAnchor="end"
...
export function KindBars({
  ...
  return (
    <div className="flex flex-col gap-s2">
      <Legend
        items={[
          { color: 'var(--accent)', text: labels.ok },
          { color: 'var(--err)', text: labels.failed },
        ]}
      />
      <div className="relative">
        <svg
          ...
          {rows.map((строка, i) => {
            ...
            return (
              <g key={строка.kind} opacity={под === null || под === i ? 1 : 0.55}>
                ...
                <rect x={ИМЯ} y={y} width={целых} height={14} rx={3} fill="var(--accent)" />
                {упало > 0 && (
                  <rect
                    x={ИМЯ + целых + 2}
                    y={y}
