 ...
 * Хуки имени и компоненты держатся в одном файле намеренно: они об одном — как
 * пространство называется на экране и что делать, когда открыто чужое.
 * Разнести их значило бы завести файл ради одной строки.
 */
/**
 ...
 *
 * `WorkspaceTag` — то же самое ростом со строку: имя пространства у каждой
 * работы в списке, а не только подписью над ним. Подпись над списком отвечает
 * на вопрос один раз и наверху, а строку работы копируют, пересылают и
 * читают в отрыве от шапки — на снимке экрана, в узком окне, после прокрутки, —
 * и тогда «где эта работа лежит» должно читаться прямо на ней.
 *
...
/**
 * Столько знает о пространстве строка работы: служба шлёт вместе с работой имя
 * пространства (`ProjectOut`), но не признак «личное ли оно».
 */
export type ProjectWorkspace = { workspace_id?: string; workspace_name?: string }

/**
 * Как назвать пространство работы. → функция, потому что зовут её в цикле по
 * списку, а хук в цикле не позовёшь.
 *
 * Признака «личное» у работы нет, а он решает, показывать ли вместо служебного
 * `Personal` ник хозяина. Признак берётся у карточки текущего пространства,
 * когда работа лежит в нём, — списки работ отобраны по текущему пространству,
 * так что это обычный случай; работа из соседнего пространства называется тем
 * именем, которое дала служба.
 */
export function useProjectWorkspaceName(): (p: ProjectWorkspace) => string {
  const текущее = useCurrentWorkspace()
  const личное = usePersonalName()
  return (p) => {
    const имя = p.workspace_name
    if (!имя) return ''
    const карточка = текущее.data
    const своё = карточка && карточка.id === p.workspace_id ? карточка : null
    return workspaceLabel({ name: имя, personal: !!своё?.personal }, личное)
  }
}

/**
 * Имя пространства строкой работы: значок и имя, приглушённо и мелко.
 *
 * `span`, а не `p`, — чтобы вставать и отдельной строкой под именем работы, и
 * внутрь строки подробностей рядом с датой и размером. Без слова
 * «Пространство»: в строке оно занимало бы больше места, чем само имя, а значок
 * тот же, что у подписи над списком, и читается тем же.
 */
export function WorkspaceTag({
  ...
  const t = useT()
  const имя = useProjectWorkspaceName()(project)
  if (!имя) return null
  return (
    <span
      title={t('workspace.switcher.caption')}
      className={cn(
        'inline-flex max-w-full min-w-0 items-center gap-1 align-middle text-xs text-muted',
        className,
      )}
    >
      <Icon name="users" size={12} className="shrink-0" />
      <span className="min-w-0 truncate">{имя}</span>
    </span>
  )
}
