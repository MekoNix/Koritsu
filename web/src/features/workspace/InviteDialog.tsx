 ...
 *
 * Приглашение по почте, а не по идентификатору: чужой uuid человеку неоткуда
 * взять, а почту коллеги он знает.
 *
 * **Писем служба не шлёт**: приглашение приходит уведомлением в самом сайте, и
 * получить его может только тот, у кого уже есть аккаунт. Поэтому у почты,
 * которой в службе нет, отказ `no_such_user`, и текст его говорит правду —
 * «такого человека нет, пусть сперва заведёт аккаунт», а не «приглашение
 * отправлено».
 *
 * Участником позванный становится, только ответив «принять»: до этого он стоит
 * в списке участников с пометкой «приглашён».
 *
