 ...
 * текущей работе: кнопка ставит запись в журнал и уводит на неё же. Схемы
    ...
    create.mutate(
      { projectId, module },
      {
        onSuccess: (run) => {
          ...
          // Ведём на сам заведённый запуск, а не на экран модуля: отчётов и
          // решений в работе несколько, и общий адрес открыл бы документ самого
          // первого из них — человек правил бы не то, что только что завёл.
          if (link) navigate(link.runHref?.(projectId, run.id) ?? link.href(projectId))
        },
