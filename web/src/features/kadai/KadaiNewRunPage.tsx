 ...
 * пережил бы вкладку. Кончились попытки — решение всё равно заведено, и
 * условие назовёт его экран при первом прогоне, как было раньше.
 */
const ПОПЫТОК = 60
const ШАГ_МС = 500
...
export function KadaiNewRunPage() {
   ...
   * означает «разбор ещё идёт», а не «ничего не получилось», и экран решения
   * назовёт условие сам, когда человек нажмёт «Решить задачу».
   *
  ...
  async function завести() {
    ...
    try {
      ...
      // него, и делается это фоном — ради имени решения. Безымянное решение
      // зовётся по файлу условия, а список решений обязан показать это имя до
      // первого прогона, иначе «Решение 3» в списке из нескольких задач ничем
      // не отличается от «Решения 4».
      //
