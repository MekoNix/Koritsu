 ...
 *    отказал бы «условие задачи не приложено». Тем же шагом решение, которому
 *    имени не дали, получает имя файла условия: «Решение 4» в списке из шести
 *    задач ничем не отличается от «Решения 5». Условие, набранное текстом,
 *    имени не даёт — файл под него придумали мы, а не человек. Шаг ждёт конца
 *    разбора и потому идёт фоном: форма отпускает человека сразу, а список
 *    решений перечитается сам.
 *
...
export function KadaiNewRunPage() {
  ...
  async function назвать_условием(materialId: string, runId: string, попыток = ПОПЫТОК) {
    try {
      await setCondition.mutateAsync({ projectId, materialId, runId, useFileName: файлом })
    } catch {
      if (попыток <= 0) return
  ...
  async function завести() {
    ...
    try {
      ...
      // решения показывает условием то, что названо им у службы, и без этого
      // шага он честно скажет, что условие не названо. Вторая: безымянное
      // решение зовётся по файлу условия, а список решений обязан показать это
      // имя до первого прогона, иначе «Решение 3» в списке из нескольких задач
      // ничем не отличается от «Решения 4».
      //
      // `useFileName` — откуда взять имя безымянному решению. Файл принёс
      // человек, и его имя говорит, какая это задача; а `.txt` с набранным
      // текстом придумали мы, и называть решение своей же выдумкой незачем.
