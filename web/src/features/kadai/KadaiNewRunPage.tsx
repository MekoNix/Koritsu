 ...
 *     ----------------------------
 *
 * Действия идут подряд, и разорвать их нельзя:
 *
 * 1. **запись решения** — `POST …/kadai/runs`: она заводит и каталог решения на
 *    томе, и строку в журнале работы;
 * 2. **условие**. Набранное текстом ложится строкой в состояние решения одним
 *    запросом (`PUT …/kadai/condition`): решают по тексту, и заворачивать его в
 *    файл значило бы гнать строку через очередь разбора обратно в строку.
 *    Принесённое файлом ложится материалом в папку решения — сам файл ценен: по
 *    нему сверяют распознанное;
 * 3. **файлы контекста** — туда же, по одному запросу на файл;
 * 4. **назвать файл условием** — до этого он обычный материал, и прогон отказал
 *    бы «условие задачи не приложено». Тем же шагом решение, которому имени не
 *    дали, получает имя файла: «Решение 4» в списке из шести задач ничем не
 *    отличается от «Решения 5». Шаг ждёт конца разбора и потому идёт фоном:
 *    форма отпускает человека сразу, а список решений перечитается сам.
 *
...
import { useCreateKadaiRun, useSetCondition, useSetConditionText, useSetKadaiWishes } from './data'
import { РЕШЕНИЕ, type KadaiScope } from './module'
import { запомнить } from './preset'

/**
 * Сколько раз пробовать назвать условие и с каким шагом (полминуты).
 *
 * Ждать приходится потому, что разбор материала уехал в очередь, а до его
 * конца материала для службы не существует. Ограничено намеренно: работа без
 * воркера не станет разобранной никогда, а вечный запрос раз в полсекунды
 * пережил бы вкладку. Кончились попытки — решение всё равно заведено и файл в
 * нём лежит: экран решения скажет, что условие не названо, и предложит выбрать
 * файл из папки одним нажатием. Подставлять его за человека там не станут — на
 * это уходит прогон, оплаченный за решение чужой задачи.
 */
const ПОПЫТОК = 60
const ШАГ_МС = 500
...
export function KadaiNewRunPage({ scope = РЕШЕНИЕ }: { scope?: KadaiScope }) {
  ...
  const идёт = создать.isPending || upload.isPending || setCondition.isPending || setText.isPending
  const готово = !!projectId && (файлом ? !!файл : !!текст.trim())

  /**
   * Назвать принятый материал условием, дождавшись разбора. Не вышло — молча.
   *
   * Молча потому, что решение уже заведено и файлы в нём лежат: беда здесь
   * означает «разбор ещё идёт», а не «ничего не получилось». Не успело за
   * отведённые попытки — условие называют на экране решения, где видно, из
   * каких файлов выбирают.
   *
   * Ход идёт **фоном**, без ожидания на форме: держать человека на ней, пока
   * распознаётся скан, незачем, а список решений перечитается сам — успешный
   * вызов гасит его ключ (`useSetCondition`).
   */
  async function назвать_условием(materialId: string, runId: string, попыток = ПОПЫТОК) {
    try {
      // `useFileName` — истина всегда: сюда попадает только файл, принесённый
      // человеком, и его имя говорит, какая это задача.
      await setCondition.mutateAsync({ projectId, materialId, runId, useFileName: true })
    } catch {
      if (попыток <= 0) return
      await new Promise((дальше) => setTimeout(дальше, ШАГ_МС))
      await назвать_условием(materialId, runId, попыток - 1)
    }
  }
  ...
  async function завести() {
    ...
    try {
      ...
      if (файлом) {
        // Файл условия и файлы контекста ложатся в папку этого решения
        // (`run_id` в форме): в промпт уезжают только они, и файл соседней
        // задачи модель не увидит.
        ...
        // него, и делается это фоном — ради двух вещей сразу. Первая: экран
        // решения показывает условие из состояния, а туда оно попадает вместе с
        // разобранным текстом файла. Вторая: безымянное решение зовётся по
        // файлу условия, а список решений обязан показать это имя до первого
        // прогона, иначе «Решение 3» в списке из нескольких задач ничем не
        // отличается от «Решения 4».
        void назвать_условием(принят.pending_id, решение.id)
      } else {
        // Условие текстом — одна строка в состоянии решения, без файла и без
        // очереди: ждать здесь нечего, и экран решения покажет его сразу.
        await setText.mutateAsync({ projectId, runId: решение.id, text: текст.trim() })
      }
