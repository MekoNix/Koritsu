...
// Имена событий лежат в `types.ts` рядом с видами заданий; обратная ссылка
// оттуда сюда — только на тип (`import type`), и кольца при сборке не будет.
import { EV_STEP } from './types'
...
/** Событие `step` из потока задания — так, как оно приезжает. */
type StepEvent = {
  stage?: unknown
  tool?: unknown
  ok?: unknown
...
/**
 * Ход внутри стадии — событие `step` потока, приведённое к форме экрана.
 *
 * Договор со службой: `{stage, tool, ok, note, n, total}`. `tool` бывает
 * пустым — у стадий без инструментов ход называется словами («читаю условие»),
 * — а `total` бывает нулём: потолок ходов известен не всегда, и полоска с
 * выдуманным знаменателем врёт (правило `StageStrip`).
 *
 * `seq` — номер кадра в потоке: он же ключ списка. Номера не повторяются, а
 * содержимое ходов повторяется легко («ход 7 из 50» дважды подряд — законный
 * случай), и ключ по содержимому склеил бы два разных хода в один.
 */
export type Move = {
  seq: number
  stage: string
...
/** Сколько последних ходов держит журнал: он для «что было только что». */
export const ЖУРНАЛ = 30

/**
 * Разобрать кадры потока в ходы.
 *
 * Свёртка по всему списку кадров, а не накопление: поток переподключается с
 * последнего события (`Last-Event-ID`), и кадры после обрыва приезжают те же
 * самые — накопитель показал бы каждый ход дважды.
 *
 * Ход без фразы отбрасывается: пустая строка в журнале — это строка, по
 * которой человеку нечего понять, а место она занимает как настоящая.
 */
export function readSteps(events: JobEvent[], limit = ЖУРНАЛ): Move[] {
  const ходы: Move[] = []
  for (const кадр of events) {
    if (кадр.kind !== EV_STEP) continue
    const тело = (кадр.data ?? {}) as StepEvent
    const note = typeof тело.note === 'string' ? тело.note.trim() : ''
    if (!note) continue
    ходы.push({
      seq: кадр.seq,
      stage: typeof тело.stage === 'string' ? тело.stage : '',
      tool: typeof тело.tool === 'string' ? тело.tool : '',
      // Пусто — это удача: отказ службе назвать проще, чем успех, и молчание
      // об исходе честнее читать как «получилось».
      ok: тело.ok !== false,
      note,
      n: typeof тело.n === 'number' ? тело.n : 0,
      total: typeof тело.total === 'number' ? тело.total : 0,
    })
  }
  return ходы.length > limit ? ходы.slice(ходы.length - limit) : ходы
}
