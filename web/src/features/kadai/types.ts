...
export type KadaiStatus = {
  ...
  /**
   * Карточка материала, названного условием сейчас. Пусто — не назвали.
   *
   * Берётся с тома, а не из хода стадий: условие называют до первого прогона, а
   * поправленное ложится новым материалом — ход стадий помнил бы прежнее до
   * следующего прогона. `ocr` знает только прогон, прочитавший файл.
   */
  condition: { material?: string; name?: string; unit?: string; ocr?: boolean }
  /** Условие так, как его прочитал прогон. Пусто — прогон его ещё не читал. */
  condition_text: string
  /**
   * Материалы, бывшие условием до нынешнего.
   *
   * Лежат в папке решения, но модели не показываются: два условия сразу,
   * прежнее и поправленное, дают решение по тому, которое человек исправил.
   */
  condition_past: string[]
