 ...
 * кафедры кладут один раз, а нужна она не каждой задаче. `selected` — уезжает
 * ли он в промпт этого решения; по умолчанию не уезжает ни один: за промпт, в
 * который файл попал сам, платит человек, не увидев его там.
 */
 ...
 *
 * Ключи `made`: `docx` и `pdf` кладёт стадия «сборка», `archive` и
 * `archive_light` — стадия «архив» (`orchestrator.kadai._положить_архив`).
 * Архива два вида: полный несёт всё, чем работа собрана, а лёгкий — только
 * отчёт, исходники и схемы. Ключа нет — файла ещё нет, и ссылки на экране тоже.
 */
export type KadaiStatus = {
  ...
  /**
   * Карточка файла, из которого условие взято. Пусто — условие набрали текстом,
   * файла у него нет вовсе.
   *
   * Берётся с тома, а не из хода стадий: файл условия называют до первого
   * прогона. `ocr` знает только прогон, прочитавший файл.
   */
  condition: { material?: string; name?: string; unit?: string; ocr?: boolean }
  /**
   * Условие задачи текстом — то самое, по которому решают.
   *
   * Текст, а не файл, потому что решают по тексту: файл (скан, `docx`, `pdf`) —
   * лишь способ его принести, и распознанное из него ложится сюда же. Правка
   * условия правит эту строку, а не заводит второй файл: два условия рядом,
   * прежнее и поправленное, решались бы по прежнему.
   */
...
/**
 * Имя события потока: один ход внутри стадии.
 *
 * `stage` говорит только о границах — стадия началась, стадия кончилась, — а
 * между ними сценарий работает минутами: решает, зовёт инструменты, пишет
 * тексты. Экран, знающий одни границы, всё это время показывает «идёт решение»
 * и молчит, и молчание читается как зависание. `step` — то, что происходит
 * между границами, фразой для человека.
 */
export const EV_STEP = 'step'
