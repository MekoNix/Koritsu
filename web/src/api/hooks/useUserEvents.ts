...
/**
 * Виды заданий, чей экран показывает и удачу, и отказ сам, на месте: сборка и
 * трасса программы на ассемблере кончаются статусом в строке меню, логом
 * «Сборки» и подсветкой в «Исходнике», чтение памяти — пометкой в «Дампе».
 * Такое задание человек запускает, глядя на этот экран, и ошибка сборки —
 * обычный ход работы, а не беда; тост поверх экрана повторял бы статус.
 * Колокольчик уведомление о сборке по-прежнему получает; о чтении памяти служба
 * уведомления не заводит вовсе (`jobs.registry.БЕЗ_УВЕДОМЛЕНИЯ`).
 */
const СВОЙ_ЭКРАН = new Set(['asm_run', 'asm_memory'])
...
export function useUserEvents(enabled = true): SseState {
  ...
  const onFrame = useCallback(
    (frame: SseFrame) => {
      ...
      if (СВОЙ_ЭКРАН.has(видЗадания(card))) return
