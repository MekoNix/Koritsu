...
export const keys = {
  ...
  projects: {
    ...
    /** Отчёты одной работы: список её карточек и заголовок экрана отчёта. */
    reports: (projectId: string) => ['projects', 'one', projectId, 'reports'] as const,
    /**
     * Отчёты всех работ пространства — лента главной модуля.
     *
     * Не под `projects.one`: работы у этого ответа нет вовсе, он про
     * пространство. Под корнем `projects` — чтобы смена пространства гасила его
     * вместе со списками работ (`useWorkspaceScopeReset`).
     *
     * Заведённый, снесённый, переименованный или собранный отчёт меняет и
     * карточку в этой ленте, поэтому мутации гасят её отдельной строкой: сброс
     * работы (`projects.one`) сюда не достаёт, и лента показывала бы прежнее до
     * перезагрузки страницы.
     */
    workspaceReports: (workspaceId: string) => ['projects', 'reports', workspaceId] as const,
    /** Та же лента любого пространства — корень для сброса. */
    workspaceReportsAll: ['projects', 'reports'] as const,
  },
