 ...
 *
 * У «Ассемблера» таких кодов два, и по той же причине: `беда_словами`
 * (`runs/handlers/common.py`) пропускает наружу русский текст ядра — «метка
 * прогона не совпала», «шаг за концом трассы», — и словарная фраза «снять дамп
 * не вышло» скрыла бы ровно то, что человек может починить. Английское и
 * машинное та же функция не пропускает, поэтому строка в поле всегда читаемая.
 */
const СВОИМИ_СЛОВАМИ = new Set(['kadai_failed', 'asm_failed', 'asm_chat_failed'])
