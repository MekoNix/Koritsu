 ...
 *
 * Что делать в областях: формы своих областей описывать так же — своим файлом
...
export type Me = {
  ...
  email: string
  /**
   * Ник — имя человека на экране. Своего имени и
   * фамилии у аккаунта нет и не будет: показывается ник, почта — мелко и
   * частично скрытой (`lib/maskEmail`).
   */
  nickname: string
  plan: string
  ...
  is_admin: boolean
  /**
   * Пресет модели, выбранный человеком по умолчанию (`null` — не выбран, и
   * тогда решает правило сайта: сначала свой ключ, потом общий). Живёт в
   * профиле, а не в браузере: выбор обязан быть тем же на втором устройстве.
   */
  default_endpoint: string | null
  /** Переписывать ли агенту значения, правленные рукой человека. */
  agent_overwrite: boolean
  /**
   * Сколько раз человек менял свою картинку. Ноль — своей картинки нет, и
   * аватар рисуется из идентификатора (`ui/Avatar`). Число едет в адрес
   * картинки: служба разрешает держать её в кэше год, и без меняющегося
   * адреса замена была бы не видна самому человеку.
   */
