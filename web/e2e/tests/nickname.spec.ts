 ...
 *
 * У службы есть поле `nickname` и `PATCH /api/auth/me`, а у сайта — места, где
 * ник заменил почту: приветствие на дашборде, меню имени в шапке, имя личного
 * пространства и строка участника. Проверять их порознь нечем: каждое по
 * отдельности — одно поле из `me`, а вместе они отвечают на вопрос
 * «переименовался ли человек везде, где его зовут по имени». Разъезжается это
 * ровно на правке ника, поэтому ник здесь и правится.
 *
 * Имя личного пространства — исключение, и оно тоже проверяется: ник даёт ему
 * имя при заведении, но дальше это обычное поле пространства, за ником оно не
 * бегает.
 *
...
test('ник: приветствие, меню, правка в профиле, участники', async ({ page }) => {
  ...
  // А вот имя личного пространства правка ника не трогает: оно собирается из
  // ника один раз, при заведении, и дальше остаётся обычным полем, которое
  // человек вправе переименовать сам. Ходить и переписывать его вслед за ником
  // значило бы затирать выбранное имя.
  await expect(переключатель).toContainText(ник)
  await expect(переключатель).not.toContainText(новый)
