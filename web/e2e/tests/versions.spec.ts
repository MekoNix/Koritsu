 ...
 * колокольчике.
 *
 * Две вещи ночи 2 в одном пути, и они действительно один путь: человек правит
...
test('версии: сравнение по словам и возврат; экспорт: файл в колокольчике', async ({ page }) => {
  ...
  // Форматов три, а заданий за ними два: Word и PDF собираются одним прогоном
  // (`build`), архив — своим (`export`). Здесь берётся архив: он собирается без
  // LibreOffice и не зависит от того, что стоит на машине.
