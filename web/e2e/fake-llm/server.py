...
отказа поставщика.
    ...
    Ходы инструментами по требованию
    --------------------------------

    [[FAKE:TOOLS]]  позвать объявленный инструмент, а следующим ходом — итог
                    словами (`ходы`)

Без него уровень 3 на подделке проходит вхолостую: инструменты объявлены, но
звать их никто не заставляет, подделка отвечает словами, и в потоке нет ни
одного `tag_closed`. Маркер в задаче агента даёт петле ровно два хода — вызовы
и итог, — и этим на стенде появляется то, что сайт показывает окном агента:
ходы, поставленные теги, `result.filled`. Вслепую подделка по-прежнему не
зовёт: инструмент выбирается тот, чьи аргументы можно заполнить из самого
запроса, а тип значения читается из описи тегов в промпте.
...
# Просьба походить инструментами (уровень 3). Отдельным выражением, а не веткой
# `МАРКЕР_RE`: те маркеры просят беду и разбираются до `придумать`, а этот —
# просит работу и разбирается вместе с остальными формами ответа.
МАРКЕР_ХОДОВ_RE = re.compile(r"\[\[FAKE:TOOLS\]\]")
ТЕГ_RE = re.compile(r"Заполни тег \[([^\]\n]+)\]")
ТЕГИ_RE = re.compile(r"Заполни (?:теги|блоки): ([^\n]+)")
КЛЮЧ_В_СКОБКАХ_RE = re.compile(r"\[([^\]\s]+)\]")

# Строка описи тегов шаблона (`hokoku.manifest`, кусок «задание из шаблона»):
#     [цель] цель — markdown (абзацы, списки, заголовки, таблицы); обязательный
# Из неё берётся тип значения тега — иначе ветку помеченного объединения
# пришлось бы угадывать, а угаданная не проходит валидатор службы.
ТИП_ТЕГА_RE = re.compile(r"^\[(?P<ключ>[^\]\n]+)\][^\n]*? — (?P<тип>[a-z_]+)", re.M)
...
# Поля-адреса. Прозой их заполнить нельзя: ключ — это адрес, а не текст, и
# знаков «{}:|#/» и пробелов в нём быть не должно (`hokoku.live.new_key`).
# Подделка про смысл поля не знает и знать не должна, но имя `key` в этой
# системе значит ровно одно, и текст на его месте роняет стадию «шаблон» у
# `kadai` на первом же разделе — то есть проверяется не путь, а отказ.
КЛЮЧИ = ("key",)
_СЧЁТЧИК_КЛЮЧЕЙ = itertools.count(1)
...
def типы_тегов(текст: str) -> dict:
    """Ключ тега → имя его типа, как их перечислила опись шаблона в промпте.

    Подделке это нужно ровно для одного: значение тега — помеченное объединение
    по всем типам сразу (`orchestrator.schema.any_value_schema`), и выбрать в
    нём ветку наугад нельзя. Тип, не совпавший с манифестом, служба отвергает
    тем же замечанием, что и у человека, — и вместо хода получился бы отказ.

    Про типы подделка ничего не знает: имя ветки она читает из запроса, как и
    всё остальное.
    """
    return {найдено.group("ключ").strip(): найдено.group("тип")
            for найдено in ТИП_ТЕГА_RE.finditer(текст)}
...
def _собрать(строки: list[str], i: int, уровень: int):
    ...
    if элементы is not None:
        # Поля объекта-элемента `describe` печатает на **том же** отступе, что и
        # строку «элементы списка», а не глубже: своего уровня у элемента нет.
        # Значит собранные здесь поля принадлежат ему, а не списку. Без этого
        # список объектов приезжает списком пустых объектов, и ответ подделки не
        # проходит валидатор службы — стадия «шаблон» у `kadai` спотыкается на
        # `нет обязательного ключа 'key'`, хотя схему ей пересказали целиком.
        if свойства and not элементы.get("properties"):
            элементы = {**элементы, "type": элементы.get("type") or "object",
                        "properties": свойства, "required": обязательные}
...
def по_схеме(схема: dict, теги: list[str], имя: str = "", метка: str = "") -> object:
    ...
    `метка` — какую ветку помеченного объединения брать (`_ветка`). Пусто —
    первую, как было: у объединения без дискриминатора выбирать не из чего.
    """
    ...
    if схема.get("enum"):
        return схема["enum"][0]
    if схема.get("anyOf"):
        return по_схеме(_ветка(схема["anyOf"], метка), теги, имя, метка)
    тип = схема.get("type")
        ...
        for ключ in свойства:
            ...
            if _только_пусто(под):
                ...
                готово[ключ] = по_схеме(
                    под, теги, предмет if ключ in СОДЕРЖАНИЕ else ключ, метка)
        return готово
    if тип == "array":
        items = схема.get("items") or {"type": "string"}
        сколько = max(1, int(схема.get("minItems") or 1))
        return [по_схеме(items, теги, имя, метка) for _ in range(сколько)]
    if тип == "integer":
    ...
    if тип == "null":
        return None
    if имя in КЛЮЧИ:
        return f"раздел-{next(_СЧЁТЧИК_КЛЮЧЕЙ)}"
    return текст_про(имя, теги)


def _ветка(варианты: list, метка: str) -> dict:
    """Ветка помеченного объединения по дискриминатору `type`.

    Значение тега объявлено объединением по всем типам сразу
    (`orchestrator.schema.any_value_schema`), и первая ветка (`text`) годится
    ровно для тега типа `text`: остальным служба ответит замечанием «тип не
    совпал с манифестом». Метку подделка не выдумывает — читает из описи тегов
    в самом промпте (`типы_тегов`).
    """
    if метка:
        for вариант in варианты:
            если = ((вариант.get("properties") or {}).get("type") or {})
            if если.get("const") == метка:
                return вариант
    return варианты[0]
...
def придумать(тело: dict) -> tuple[str, list[dict]]:
    ...
    if МАРКЕР_ХОДОВ_RE.search(json.dumps(тело, ensure_ascii=False)) \
            and (тело.get("tools") or []):
        return ходы(тело, теги, текст_запроса)

    # Инструменты объявлены, но никого звать не заставляли (уровень 3), и
    # маркера ходов нет: отвечаем словами и тем заканчиваем петлю. Звать
    # инструменты «за модель» подделке нечем — их смысл она не знает, а
    # угаданный вызов записал бы в проект чушь.
    return прозой(текст_запроса, теги), []


# ── ходы инструментами по маркеру [[FAKE:TOOLS]] ─────────────────────────────

def ходы(тело: dict, теги: list[str], текст_запроса: str) -> tuple[str, list[dict]]:
    """Один ход инструментами, потом итог словами. → (текст, вызовы).

    Зачем это есть. Без принуждения (`tool_choice`) подделка отвечает словами, и
    прогон уровня 3 проходит, но не ставит ни одного значения: в потоке нет ни
    `tag_closed`, ни `result.filled`, и проверить сайтом нечего. Маркер
    `[[FAKE:TOOLS]]` в задаче агента просит её сделать то, что делает модель:
    позвать объявленный инструмент, а следующим ходом — закончить.

    Ходов ровно два, и второй узнаётся из самого запроса, а не из памяти
    (`уже_ходили`): состояния между запросами у сервера нет, а маркер стоит в
    задаче и приезжает на каждом ходу петли. Не различай — петля звала бы
    инструмент до потолка ходов, и прогон кончался бы `interrupted`.
    """
    выбор = _инструмент_для_ходов(тело)
    if выбор is None or уже_ходили(тело):
        return итог_ходов(теги, выбор[0] if выбор else ""), []
    имя, схема = выбор
    типы = типы_тегов(текст_запроса)
    вызовы = []
    for номер, ключ in enumerate(теги[:2] or [""], 1):
        аргументы = _аргументы(схема, ключ, типы.get(ключ, ""))
        вызовы.append({"id": f"call_fake_{номер}", "name": имя,
                       "arguments": json.dumps(аргументы, ensure_ascii=False)})
    return "", вызовы


def уже_ходили(тело: dict) -> bool:
    """Были ли ходы до этого — по ответам инструментов в истории запроса.

    Петля кладёт ответ каждого инструмента обычным сообщением `role: tool`
    (`llm.backends.openai_compat._history_json`), и другого признака «этот
    запрос не первый» в теле нет.
    """
    return any((с.get("role") or "") == "tool" for с in тело.get("messages") or [])


def _инструмент_для_ходов(тело: dict):
    """Какой из объявленных инструментов подделка может позвать не вслепую.

    Годится тот, чьи обязательные аргументы заполняются из самого запроса.
    Инструмент без аргументов до петли не доезжает вовсе (`orchestrator.tools`:
    вызов с `{}` она считает битым JSON), а идентификатор материала или
    исходник программы подделке взять неоткуда — выдуманные дали бы отказ
    инструмента вместо хода. Остаётся тот, что просит `key`: ключи названы в
    промпте («Заполни теги: [цель], [выводы]»), и их подделка уже читает.
    """
    годные = []
    for инструмент in тело.get("tools") or []:
        функция = инструмент.get("function") or {}
        схема = функция.get("parameters") or {}
        if схема.get("properties"):
            годные.append((функция.get("name") or "", схема))
    for имя, схема in годные:
        if set(схема["properties"]) & set(КЛЮЧИ):
            return имя, схема
    return годные[0] if годные else None


def _аргументы(схема: dict, ключ: str, тип: str) -> dict:
    """Аргументы одного вызова: поле-адрес — названным ключом, остальное — по схеме.

    Поле собирается отдельно от `по_схеме`, а не правится после неё, ради текста
    на экране: `по_схеме` пишет «раздел «value»» там, где поле называется
    `value`, а проверяющему надо видеть «раздел «цель»».
    """
    свойства = схема.get("properties") or {}
    нужны = схема.get("required") or list(свойства)
    готово: dict = {}
    for поле in свойства:
        if поле not in нужны:
            continue
        if поле in КЛЮЧИ and ключ:
            готово[поле] = ключ
        else:
            готово[поле] = по_схеме(свойства[поле], [ключ] if ключ else [],
                                    ключ, тип)
    return готово


def итог_ходов(теги: list[str], имя: str) -> str:
    """Итог словами после ходов: им петля и заканчивается."""
    про = ", ".join(теги[:2]) if теги else "работу"
    звали = f" вызовами {имя}" if имя else ""
    return (f"Поддельная модель отработала маркер [[FAKE:TOOLS]]: значения "
            f"тегов {про} поставлены{звали}, звать инструменты больше не нужно. "
            "Текст собран сервером web/e2e/fake-llm и содержания работы в себе "
            "не несёт — проверяется путь от кнопки до поставленного значения.")
